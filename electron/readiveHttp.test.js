...
test('Readive HTTPS verifies pinned identity, emits bounded responses, resumes ranges, and denies stale/cancelled assets', async t => {
    ...
    const deferredBytes = Buffer.alloc(256 * 1024, 37);
    await fs.writeFile(path.join(root, 'deferred.cbz'), deferredBytes);
    const started = await request(`/libraries/${libraryId}/prepare`, { token, method: 'POST', body: { paths: ['deferred.cbz'], manifestVersion: 2 } });
    await service.preparations.get(started.json.scanId).promise;
    const prepared = (await request(`/libraries/${libraryId}/preparations/${started.json.scanId}`, { token })).json;
    assert.equal(prepared.manifest.version, 2);
    assert.equal(prepared.manifest.files[0].sha256, null);
    const deferredRoute = `/jobs/${prepared.job.id}`;
    const deferredId = prepared.manifest.files[0].id;
    assert.equal((await request(`${deferredRoute}/accept`, { token, method: 'POST', body: { manifestId: prepared.manifest.id, largeConfirmed: true } })).status, 200);
    assert.equal((await request(`${deferredRoute}/resolve/${deferredId}`, { token, method: 'POST', body: {} })).json.state, 'scanning');
    await Promise.all([...service.fileHashRequests.values()].map(value => value.promise));
    const resolved = (await request(`${deferredRoute}/resolve/${deferredId}`, { token, method: 'POST', body: {} })).json;
    const downloaded = await request(`${deferredRoute}/files/${deferredId}`, { token });
    assert.equal(downloaded.status, 200);
    assert.equal(crypto.createHash('sha256').update(downloaded.bytes).digest('hex'), resolved.file.sha256);
    assert.deepEqual(downloaded.bytes, deferredBytes);
    await service.cancel({ jobId: prepared.job.id });
    const beforeReadingJobs = service.store.state.jobs.length;
    const reader = await request(`/libraries/${libraryId}/read`, { token, method: 'POST', body: { path: 'one.txt' } });
    assert.equal(reader.status, 200);
    assert.match(reader.json.sourceVersion, /^[a-f0-9]{64}$/);
    assert.equal(service.store.state.jobs.length, beforeReadingJobs);
    const readerRoute = `/readers/${reader.json.id}/content`;
    assert.equal((await request(readerRoute)).status, 401);
    const readingBytes = await request(readerRoute, { token });
    assert.equal(readingBytes.bytes.toString(), 'changed');
    assert.equal(readingBytes.headers.etag, `"${reader.json.sourceVersion}"`);
    const readingRange = await request(readerRoute, { token, headers: { Range: 'bytes=1-3' } });
    assert.equal(readingRange.status, 206);
    assert.equal(readingRange.bytes.toString(), 'han');
    assert.equal(readingRange.headers['content-range'], 'bytes 1-3/7');
    assert.equal((await request(readerRoute, { token, headers: { Range: 'bytes=0-9007199254740992' } })).status, 416);
    assert.equal((await request(`/readers/${reader.json.id}/close`, { token, method: 'POST' })).status, 200);
    assert.equal((await request(readerRoute, { token })).status, 409);

    const destinationWaiting = service.requestDestinationPage({ deviceId: 'phone-test', parentId: null });
    const destinationRequest = (await request('/destination-requests', { token })).json.requests[0];
    assert.equal((await request('/destination-requests')).status, 401);
    assert.equal((await request(`/destinations/${destinationRequest.id}`, { token, method: 'POST', body: { name: 'Phone', parentId: null, entries: [], nextCursor: null, revision: 'root-v1' } })).status, 200);
    assert.equal((await destinationWaiting).revision, 'root-v1');

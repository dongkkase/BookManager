...
TXT는 메타데이터 관리 화면에서 책 정보를 검색·편집하고 앱 DB에 저장합니다. 표지는 로컬 이미지 또는 검색 결과에서 선택하며 `BookManagerData/text-thumbnails` 폴더에 보관합니다. 원본 TXT의 내용은 수정하지 않습니다.

TXT 연결은 파일 전체 내용의 SHA-256 해시를 기준으로 합니다. 파일 이름이나 경로가 바뀌어도 다시 추가하거나 스캔하면 저장된 메타데이터와 표지가 연결됩니다. 내용이 완전히 같은 복사본은 메타데이터를 공유하며, 본문이나 인코딩 등 파일 내용이 달라지면 별개의 파일로 취급합니다. TXT 정보와 표지를 함께 백업하려면 앱을 종료한 뒤 `BookManagerData/library.db`와 `BookManagerData/text-thumbnails` 폴더를 보관하세요.

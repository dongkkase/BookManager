...
from config import TOOL_CWEBP, TOOL_JPEGTRAN, TOOL_PNGQUANT
...
class RenameTask:
    ...
    def generate_new_name(self, index, ext, total_count, stem_name):
        ...
        if self.pattern_val == t_patterns[1]:
            ...
            safe_stem = stem_name.replace(" ", "_")
            if index == 0:
                return f"{safe_stem}_Cover{ext}"
            else:
                return f"{safe_stem}_Page_{n:0{pad}d}{ext}"
    ...
    def _phase1_convert(self, temp_dir, old_n, tmp_n, cap_opt, exif_opt):
        ...
        needs_processing = (
            (self.webp_conversion and not is_already_webp) or cap_opt or exif_opt
        )
                ...
                try:
                    ...
                    cover_entry = next(
                        (
                            e
                            for e in entries
                            if os.path.basename(e["filename"])
                            .lower()
                            .startswith("cover")
                        ),
                        None,
                    ...
                    has_non_webp = any(
                        not e["original_name"].lower().endswith(".webp")
                        for e in entries
                    )
                    ...
                    for count, entry in enumerate(entries):
                        ...
                        new_basename = self.generate_new_name(
                            count, ext, total_count, stem_name
                        )
                        new_name = (
                            new_basename
                            if self.flatten_folders
                            else os.path.join(dir_name, new_basename).replace("\\", "/")
                        )
                    ...
                    must_extract = (
                        actual_webp_needed
                        or format_changed
                        or self.flatten_folders
                        or (ext_type not in [".zip", ".cbz"])
                        or cap_opt
                        or exif_opt
                    )
                    ...
                    if not must_extract:
                        ...
                        temp_rn_archive = os.path.join(
                            sys_temp, f"ComicZIP_RN_{safe_id}_{filename}"
                        )
                        ...
                        for i in range(0, len(flat_args), 40):
                            ...
                            try:
                                subprocess.run(
                                    [self.seven_z_exe, "rn", temp_rn_archive]
                                    + flat_args[i : i + 40],
                        ...
                        if rename_success and not self._is_cancelled:
                            ...
                            if os.path.exists(temp_rn_archive):
                                os.remove(temp_rn_archive)
                            must_extract = True
                    ...
                    if must_extract and not self._is_cancelled:
                        ...
                        temp_dir = os.path.join(
                            sys_temp, f"ComicZIP_{safe_id}_{filename}"
                        )
                        ...
                        with concurrent.futures.ThreadPoolExecutor(
                            ...
                            for i, future in enumerate(
                                concurrent.futures.as_completed(future_to_args)
                            ):
                                if self._is_cancelled:
                                    break
                                ...
                                if i % max(1, len(temp_rename_mapping) // 20) == 0:
                                    p_msg = (
                                        f"Converting ({self.max_threads} Threads): {filename}"
                                        if self.lang == "en"
                                        else f"다중 코어 변환 중 ({self.max_threads} 스레드): {filename}"
                                    )
                                    self.signals.progress.emit(
                                        int((idx / total) * 100)
                                        + int(
                                            (i / len(temp_rename_mapping))
                                            * (100 / total)
                                        ),
                                        p_msg,
                                    )
                        ...
                        if not self._is_cancelled:
                            for old_n, tmp_n, new_n in temp_rename_mapping:
                                ...
                                if (
                                    ...
                                    and not converted
                                    and not old_n.lower().endswith(".webp")
                                ):
                        ...
                        self.signals.progress.emit(
                            int(((idx + 0.9) / total) * 100),
                            f"Re-archiving: {filename}",
                        )

                        target_final_path = str(Path(file_path).with_suffix(target_ext))
                        temp_archive = os.path.join(
                            sys_temp, f"ComicZIP_Done_{safe_id}_{filename}{target_ext}"
                        )
                        ...
                        is_same_path = os.path.normcase(
                            os.path.abspath(file_path)
                        ) == os.path.normcase(os.path.abspath(target_final_path))

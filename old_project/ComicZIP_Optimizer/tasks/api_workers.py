...
class ImageLoadThread(QThread):
    ...
    def run(self):
        ...
        try:
            resp = requests.get(
                self.url, headers={"User-Agent": "Mozilla/5.0"}, timeout=5
            )
            if resp.status_code == 200:
                ...
                try:
                    with sqlite3.connect(DB_PATH, timeout=10) as conn:
                        c = conn.cursor()
                        c.execute(
                            "INSERT OR REPLACE INTO img_cache (url, data) VALUES (?, ?)",
                            (self.url, img_data),
                        )
                        conn.commit()
                except:
...
class SearchWorker(QThread):
    ...
    def run(self):
        results, actual_query = MetaApiFetcher.search(
            self.api_name, self.query, self.api_keys, self.page
        )
    ...
    def run(self):
        ...
        try:
            with sqlite3.connect(DB_PATH, timeout=10) as conn:
                c = conn.cursor()
                c.execute(
                    "CREATE TABLE IF NOT EXISTS trans_cache (original TEXT PRIMARY KEY, translated TEXT)"
                )
                for f in fields_to_translate:
                    ...
                    c.execute(
                        "SELECT translated FROM trans_cache WHERE original=?",
                        (cache_key,),
                    )
                    row = c.fetchone()
                    if row and row[0]:
            ...
            uncached_data = {
                f: translated_data[f]
                for f in fields_to_translate
                if translated_data.get(f) and translated_data.get(f) != "-"
            }
        ...
        if ai_enabled and ai_key:
            ...
            try:
                ...
                if ai_provider == "OpenAI":
                    ...
                    payload = {
                        ...
                        "messages": [
                            {"role": "system", "content": prompt},
                            {
                                "role": "user",
                                "content": json.dumps(
                                    uncached_data, ensure_ascii=False
                                ),
                            },
                        ],
                        "temperature": 0.3,
                    ...
                    if resp.status_code == 200:
                        res_text = resp.json()["choices"][0]["message"][
                            "content"
                        ].strip()
                    ...
                    resp = requests.post(url, headers=headers, json=payload, timeout=15)
                    if resp.status_code == 200:
                        res_text = resp.json()["candidates"][0]["content"]["parts"][0][
                            "text"
                        ].strip()
                        ...
                        with sqlite3.connect(DB_PATH, timeout=10) as conn:
                            ...
                            for k, original_val in uncached_data.items():
                                ...
                                if translated_val:
                                    ...
                                    c.execute(
                                        "INSERT OR REPLACE INTO trans_cache (original, translated) VALUES (?, ?)",
                                        (
                                            f"{original_val}::lang_{self.target_lang}",
                                            translated_val,
                                        ),
                                    )
                            conn.commit()

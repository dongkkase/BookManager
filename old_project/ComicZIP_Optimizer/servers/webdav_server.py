...
class WebDAVServerThread(BaseServerThread):
    ...
    def stop(self):
        ...
        self.log_signal.emit(
            f"WebDAV 서버가 성공적으로 중지되었습니다. (포트 {self.port})"
        )
    ...
    def _start_server(self):
        ...
        if not lib_folders:
            self.error_signal.emit(
                "공유할 라이브러리 폴더가 없습니다. [폴더] 탭의 ⚙️ 설정에서 라이브러리를 먼저 추가해주세요."
            )
            return
        ...
        provider_mapping = {"/": self.vroot_dir}
        ...
        try:
            ...
            self.log_signal.emit(
                f"WebDAV 서버가 시작되었습니다. (포트: {self.port}, 계정: {username})"
            )
            self.server.start()
        except Exception as e:

    ...
    def _start_server(self):
        ...
        class OPDSHandler(http.server.SimpleHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if parsed_url.path == "/opds" or parsed_url.path == "/opds/":
                    ...
                    log_dir = (
                        os.path.basename(current_dir.rstrip(os.sep))
                        if current_dir
                        else "최상위 라이브러리"
                    )
                    server_thread.log_signal.emit(
                        f"[OPDS] 탐색 중: {log_dir} ({self.client_address[0]})"
                    )
                    ...
                    if not current_dir:
                        ...
                        prefix = (
                            ...
                            else current_dir + os.sep
                        )
                        cursor.execute(
                            "SELECT path, title, summary, writer, mtime, thumb_path, size, ext, page_count FROM files WHERE path LIKE ?",
                            (prefix + "%",),
                        )
                        ...
                        for row in cursor.fetchall():
                            ...
                            if len(parts) > 1:
                                ...
                                if (
                                    len(parts) == 2
                                    and folder_name not in folder_thumbnails
                                ):
                    ...
                    feed_title = (
                        f"Folder: {os.path.basename(current_dir.rstrip(os.sep))}"
                        if current_dir
                        else "ComicZIP Library"
                    )
                    ...
                    # 네비게이션 링크
                    current_url = (
                        f"/opds?dir={urllib.parse.quote(current_dir)}"
                        if current_dir
                        else "/opds"
                    )
                    xml.append(
                        f'<link rel="self" type="application/atom+xml;profile=opds-catalog;kind=acquisition" href="{current_url}"/>'
                    )
                    xml.append(
                    ...
                    if current_dir:
                        ...
                        if current_dir in registered_folders:
                            ...
                            if (
                                parent_dir == current_dir.rstrip(os.sep)
                                or parent_dir == current_dir
                            ):
                                parent_url = "/opds"
                            else:
                                if (
                                    not parent_dir.endswith(os.sep)
                                    and len(parent_dir) == 2
                                    and parent_dir.endswith(":")
                                ):
                                    parent_dir += os.sep
                                parent_url = (
                                    f"/opds?dir={urllib.parse.quote(parent_dir)}"
                                )
                        xml.append(
                            f'<link rel="up" href="{parent_url}" type="application/atom+xml;profile=opds-catalog;kind=navigation" title="Up"/>'
                        )

                    # 1. 폴더 목록 렌더링
                    for folder in sorted(folders):
                        folder_path = (
                            os.path.join(current_dir, folder) if current_dir else folder
                        )
                        encoded_folder_path = urllib.parse.quote(folder_path)
                        folder_url = f"/opds?dir={encoded_folder_path}"
                        entry_id = (
                            f"urn:uuid:{uuid.uuid5(uuid.NAMESPACE_URL, folder_url)}"
                        )
                        ...
                        xml.append(f"<updated>{now_iso}</updated>")
                        xml.append(
                            f'<link rel="subsection" type="application/atom+xml;profile=opds-catalog;kind=navigation" href="{folder_url}"/>'
                        )
                        ...
                        if current_dir and folder in folder_thumbnails:
                            ...
                            cursor.execute(
                                "SELECT path, thumb_path FROM files WHERE path LIKE ? AND path NOT LIKE ? LIMIT 1",
                                (
                                    folder_prefix + "%",
                            ...
                            if first_file:
                                f_path, f_thumb = first_file

                        if f_path:
                            if f_thumb and os.path.exists(f_thumb):
                                folder_thumb_url = (
                                    f"/download?file={urllib.parse.quote(f_thumb)}"
                                )
                            else:
                                folder_thumb_url = (
                                    f"/thumbnail?file={urllib.parse.quote(f_path)}"
                                )
                            xml.append(
                                f'<link rel="http://opds-spec.org/image/thumbnail" href="{folder_thumb_url}" type="image/jpeg" />'
                            )
                            xml.append(
                                f'<link rel="http://opds-spec.org/image" href="{folder_thumb_url}" type="image/jpeg" />'
                            )
                    ...
                    for row in files:
                        ...
                        p_count = (
                            page_count
                            if page_count and str(page_count).isdigit()
                            else "0"
                        )
                        ...
                        stream_url_template = (
                            f"/page?file={encoded_path}&amp;page_num={{pageNumber}}"
                        )
                        ...
                        xml.append(
                            f'<extent xmlns="http://purl.org/dc/terms/">{extent_str}</extent>'
                        )
                        xml.append(
                        ...
                        xml.append(
                            f'<link rel="http://opds-spec.org/image/thumbnail" href="{thumb_url}" type="image/jpeg" />'
                        )
                        xml.append(
                            f'<link rel="http://opds-spec.org/image" href="{thumb_url}" type="image/jpeg" />'
                        )

                        # PSE(스트리밍) 및 다운로드 링크 주입
                        if p_count != "0":
                            xml.append(
                                f'<link xmlns:p5="http://vaemendis.net/opds-pse/ns" rel="http://opds-spec.org/acquisition/open-access" type="{mime_type}" href="{download_url}" p5:count="{p_count}"/>'
                            )
                            xml.append(
                                f'<link xmlns:p5="http://vaemendis.net/opds-pse/ns" rel="http://vaemendis.net/opds-pse/stream" type="image/jpeg" href="{stream_url_template}" p5:count="{p_count}"/>'
                            )
                        else:
                            xml.append(
                                f'<link rel="http://opds-spec.org/acquisition/open-access" type="{mime_type}" href="{download_url}"/>'
                            )
                            xml.append(
                                f'<link xmlns:p5="http://vaemendis.net/opds-pse/ns" rel="http://vaemendis.net/opds-pse/stream" type="image/jpeg" href="{stream_url_template}"/>'
                            )
                ...
                # 2. 실제 파일 다운로드(스트리밍) 요청 처리
                    ...
                    if "file" in params:
                        ...
                        server_thread.log_signal.emit(
                            f"[OPDS] 📥 다운로드 시작: {file_name} ({self.client_address[0]})"
                        )
                ...
                # 4. 페이지 스트리밍 피드 요청 처리 (PSE)
                    ...
                    if "file" in params:
                        file_name = os.path.basename(params["file"][0])
                        server_thread.log_signal.emit(
                            f"[OPDS] 📱 스트리밍 시작: {file_name} ({self.client_address[0]})"
                        )
            ...
            def _serve_thumbnail(self, file_path):
                    ...
                    if ext in ["zip", "cbz"]:
                        with zipfile.ZipFile(file_path, "r") as zf:
                            images = [
                                ...
                                if f.lower().endswith(img_exts) and "__MACOSX" not in f
                            ]
                            images.sort()
                            if images:
                                cover = images[0]
                        ...
                        output = subprocess.check_output(
                            cmd, startupinfo=startupinfo, text=True, errors="ignore"
                        )
                        images = [
                            line[7:].strip()
                            for line in output.splitlines()
                            if line.startswith("Path = ")
                            and line.lower().endswith(img_exts)
                            and "__MACOSX" not in line
                        ]
                        images.sort()
                        if images:
                            cover = images[0]
                            cmd_ext = [TOOL_7Z, "e", "-so", file_path, cover]
                            cover_data = subprocess.check_output(
                                cmd_ext, startupinfo=startupinfo
                            )
                            cover_ext = cover.lower()
                ...
                if cover_data:
                    self.send_response(200)
                    if cover_ext.endswith(".png"):
                        mime = "image/png"
                    elif cover_ext.endswith(".webp"):
                        mime = "image/webp"
                    elif cover_ext.endswith(".gif"):
                        mime = "image/gif"
                    else:
                        mime = "image/jpeg"
            ...
            def _serve_stream_feed(self, file_path):
                ...
                xml.append(
                    f"<id>urn:uuid:{uuid.uuid5(uuid.NAMESPACE_URL, file_path + '_stream')}</id>"
                )
                xml.append(
                    f"<title>{html.escape(os.path.basename(file_path))} - Pages</title>"
                )
                xml.append(f"<updated>{now_iso}</updated>")
                ...
                for i, img in enumerate(images):
                    ...
                    xml.append(
                        f"<id>urn:uuid:{uuid.uuid5(uuid.NAMESPACE_URL, file_path + img)}</id>"
                    )
                    xml.append(f"<updated>{now_iso}</updated>")
                    ...
                    xml.append(
                        f'<link rel="http://opds-spec.org/acquisition" href="{page_url}" type="{mime}" />'
                    )
                    xml.append("</entry>")
        ...
        try:
            ...
            self.log_signal.emit(
                f"[OPDS] Server listening at http://{local_ip}:{self.port}/opds"
            )

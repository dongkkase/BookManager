...
def load_config():
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                default_config["max_threads"] = min(
                    default_config.get("max_threads", default_threads), safe_max
                )
    except:
        pass
    ...
    default_config["font_family_str"] = (
        "'Jua', 'arial', 'Noto Sans KR', 'Segoe UI Emoji'"
        if ff == "Default"
        else f"'{ff}', 'Segoe UI Emoji'"
    )

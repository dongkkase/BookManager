    ...
    def get_target_index(self, target_folder):
        with self.lock:
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT full_path, name, size FROM dup_target_index WHERE target_folder = ?",
                    (target_folder,),
                )
    ...
    def remove_target_index_bulk(self, paths):
        ...
        with self.lock:
            try:
                ...
                cursor = conn.cursor()
                cursor.executemany(
                    "DELETE FROM dup_target_index WHERE full_path = ?",
                    [(p,) for p in paths],
                )
                conn.commit()
            except Exception as e:
    ...
    def get_dup_match(self, a_path):
        with self.lock:
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT match_data FROM dup_cache WHERE a_path = ?", (a_path,)
                )
    ...
    def save_dup_matches_bulk(self, match_list):
        ...
        with self.lock:
            try:
                ...
                records = [
                    (m[0], json.dumps(m[1], ensure_ascii=False)) for m in match_list
                ]
                cursor.executemany(
                    """
                    INSERT OR REPLACE INTO dup_cache (a_path, match_data)

...
def clean_display_title(text):
    cleaned = str(text)
    cleaned = re.sub(
        r"[\[\(](번외편?|외전|스핀오프|특별편?|단편|합본)[\]\)]", r" \1 ", cleaned
    )
    ...
    cleaned = re.sub(
        r"(?:\s|^)[가-힣a-zA-Z]+\s*(?:원작|그림|지음|작화|스토리|번역)(?=\s|$)",
        " ",
        cleaned,
    )
    ...
    cleaned = re.sub(
        r"\d+(?:\.\d+)?\s*[~-]\s*\d+(?:\.\d+)?\s*(?:권|화|장|편|부)?", " ", cleaned
    )
...
def extract_core_title(text):
    ...
    delimiter_regex = re.compile(
        r"(\d{3,4}\s*px|\d+\s*(?:권|화|부(?!터))?\s*[~-]\s*\d+|\d+\s*(?:권|화|부(?!터)|화씩)|완결|\s완(\s|$))",
        re.IGNORECASE,
    )
    ...
    cleaned = re.sub(
        r"19\)|19금|19\+|15\)|15금|15\+|N새글|고화질|저화질|무료|워터마크없음|워터마크|고화질판|저화질판|단권|연재본|화질보정|확인불가",
        "",
        cleaned,
    )
    cleaned = re.sub(
        r"스캔 단면|스캔단면|스캔 양면|스캔양면|스캔본|스캔판|단편 만화|단편만화|단편(?!선)|단행본",
        "",
        cleaned,
    )
...
def is_garbage_folder_name(text):
    ...
    if "gigafile" in text_lower or "down" in text_lower:
        return True
    if len(text) > 20 and bool(re.match(r"^[a-zA-Z0-9\-_]+$", text)):
        return True
    if len(text) > 15 and bool(re.match(r"^[a-fA-F0-9\-_]+$", text)):
        return True
    if bool(re.match(r"^\d+$", re.sub(r"[\[\(].*?[\]\)]", "", text).strip())):
        return True
    return False
...
def resolve_titles(filepath, inner_name=""):
    ...
    if is_garbage_folder_name(file_stem) or not re.search(r"[가-힣a-zA-Z]", file_stem):
        ...
        if (
            inner_name
            and not is_garbage_folder_name(inner_name)
            and re.search(r"[가-힣a-zA-Z]", inner_name)
        ):
            inner_disp = clean_display_title(inner_name)
            inner_core = extract_core_title(inner_name)

        parent_is_generic = (
            parent_name.lower() in generic_folders
            or parent_disp.lower() in generic_folders
            or is_garbage_folder_name(parent_name)
        )

        if not parent_is_generic and inner_core:
            if (
                parent_core.replace(" ", "") in inner_core.replace(" ", "")
                or get_similarity(parent_core, inner_core) >= 0.4
            ):
        ...
        if (
            grandparent_name
            and grandparent_name.lower() not in generic_folders
            and not is_garbage_folder_name(grandparent_name)
        ):
...
def format_leaf_name(parent_core, leaf_name, index, total_items, lang="ko"):
    ...
    leaf_clean = re.sub(
        r"\.(zip|cbz|cbr|rar|7z)$", "", str(leaf_name), flags=re.IGNORECASE
    ).strip()
    ...
    if is_hash or not re.search(r"[가-힣a-zA-Z]", leaf_clean):
        ...
        clean_for_nums = re.sub(
            r"\d+(?:\.\d+)?\s*(?:px|p|pt|mb|gb|kb|k)(?![a-zA-Z])",
            "",
            clean_for_nums,
            flags=re.IGNORECASE,
        )
        ...
        return (
            f"{base} v{padded_num}".strip()
            if lang == "en"
            else f"{base} {padded_num}권".strip()
        )
    ...
    vol_match = re.search(
        r"(?:제|v|vol\.?\s*)?(\d+(?:\.\d+)?(?:[~-]\d+(?:\.\d+)?)?)\s*(권|화|장|편|부)",
        leaf_clean,
        re.IGNORECASE,
    )
    ...
    if "~" in rem_num_str:
        rem_num_str = rem_num_str.split("~")[0]
    if "-" in rem_num_str:
        rem_num_str = rem_num_str.split("-")[0]
    ...
    if (
        leaf_core_check
        and parent_core_check
        and get_similarity(leaf_core_check, parent_core_check) >= 0.5
    ):
        base_name = re.sub(r"^[._\-\s]+", "", parent_core)
    else:
        if (
            leaf_core_check
            and re.search(r"[가-힣a-zA-Z]", leaf_core_check)
            and not re.match(
                r"^(제|v|vol|ch|chapter|part|권|화|장|편|부)\.?\s*\d*$",
                leaf_core_check,
                re.IGNORECASE,
            )
        ):
    ...
    if lang == "en":
        if target_unit == "부":
            unit_str = f"Part {padded_num}"
        elif target_unit == "화":
            unit_str = f"Ch {padded_num}"
        else:
            unit_str = f"v{padded_num}"
    else:
        unit_str = (
            f"{padded_num}{target_unit}"
            if target_unit in ["권", "화", "장", "편", "부"]
            else f"{padded_num}권"
        )

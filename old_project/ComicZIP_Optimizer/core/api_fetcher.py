...
class MetaApiFetcher:
    ...
    def get_ridi_publish_date(b_id):
        ...
        try:
            ...
            if resp.status_code == 200:
                match = re.search(
                    r"(\d{4})\s*[./-]\s*(\d{1,2})\s*[./-]\s*(\d{1,2})(?:<[^>]+>|\s|&nbsp;|)*출간",
                    resp.text,
                )
                if match:
                    y, m, d = match.groups()
                    ...
                    with sqlite3.connect(DB_PATH, timeout=10) as conn:
                        c = conn.cursor()
                        c.execute(
                            "INSERT OR REPLACE INTO ridi_date_cache (b_id, pub_date) VALUES (?, ?)",
                            (b_id, pub_date),
                        )
                        conn.commit()
                    return pub_date
    ...
    def search(api_name, query, api_keys=None, page=1):
        ...
        with sqlite3.connect(DB_PATH, timeout=10) as conn:
            c = conn.cursor()
            c.execute(
                "SELECT results, updated_at FROM search_cache WHERE api=? AND query=?",
                (api_name, cache_query),
            )
            row = c.fetchone()
            if row:
                try:
        ...
        if api_name == "리디북스":
            ...
            results = MetaApiFetcher._search_aladin(
                query, api_keys.get("aladin", ""), page
            )
        elif api_name == "Google Books":
            results = MetaApiFetcher._search_google_books(
                query, api_keys.get("google", ""), page
            )
        elif api_name == "Anilist":
            results, is_rate_limited = MetaApiFetcher._search_anilist(
                actual_query, page
            )
        elif api_name == "Vine":
            results, is_rate_limited = MetaApiFetcher._search_vine(
                actual_query, api_keys.get("vine", ""), page
            )
        ...
        if results:
            with sqlite3.connect(DB_PATH, timeout=10) as conn:
                c = conn.cursor()
                c.execute(
                    "INSERT OR REPLACE INTO search_cache (api, query, results, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
    ...
    def _search_anilist(query, page=1):
        ...
        try:
            resp = requests.post(
                url, json={"query": graphql_query, "variables": variables}, timeout=15
            )
            if resp.status_code == 429:
                return [], True
            if resp.status_code == 200:
                for item in (
                    resp.json().get("data", {}).get("Page", {}).get("media", [])
                ):
                    ...
                    desc = (
                        re.sub(r"<[^>]+>", "", item.get("description", "")).strip()
                    ...
                    results.append(
                        {
                            "Title": title,
                            "Writer": ", ".join(writer),
                            "Penciller": ", ".join(penciller),
                            "Publisher": "",
                            "Summary": desc,
                            "Series": title,
                            "Web": "",
                            "CoverUrl": item.get("coverImage", {}).get(
                                "extraLarge", ""
                            ),
                            "Tags": ", ".join(
                            ...
                            "LocalizedSeries": title_dict.get("native", title),
                            "Count": str(
                                item.get("volumes") or item.get("chapters") or ""
                            ),
        ...
        try:
            ...
            if resp.status_code == 200:
                for item in resp.json().get("items", []):
                    ...
                    desc = (
                        re.sub(r"<[^>]+>", "", v_info.get("description", "")).strip()
                    ...
                    results.append(
                        {
                            ...
                            "Writer": ", ".join(v_info.get("authors", [])),
                            "Publisher": v_info.get("publisher", ""),
                            "Summary": desc,
                            "Series": v_info.get("title", ""),
                            "Web": v_info.get("infoLink", ""),
                            "CoverUrl": v_info.get("imageLinks", {})
                            .get("thumbnail", "")
                            .replace("http:", "https:"),
                            "Tags": categories,
                            "Genre": categories,
                            "LocalizedSeries": v_info.get("title", ""),
                            "Count": "",
                            "Rating": "-",
                            "RatingScore": "-",
                            "CommunityRating": "",
                            ...
                            "Number": "",
                            "Characters": "",
                            "PageCount": str(v_info.get("pageCount", "")),
                        }
                    )
        except:
            pass
                    ...
                    results.append(
                        {
                            ...
                            "Web": item.get("site_detail_url", ""),
                            "CoverUrl": cover_obj.get("medium_url", "")
                            if isinstance(cover_obj, dict)
                            else "",
                            "Tags": "",
                            "Genre": "",
                            "LocalizedSeries": item.get("name", ""),
                            "Count": str(item.get("count_of_issues", "")),
                            "Rating": "-",
                            "RatingScore": "-",
                            "CommunityRating": "",
                            "AgeRating": "",
                            "PubDate": str(item.get("start_year", "")),
            ...
            if response.status_code == 200:
                for item in response.json().get("item", []):
                    ...
                    for part in author_raw.split(","):
                        ...
                        name = (
                            name_match.group(1).strip() if name_match else part.strip()
                        )

                        if "(옮긴이)" in part or "(번역)" in part:
                            continue
                        elif (
                            "(그림)" in part or "(일러스트)" in part or "(작화)" in part
                        ):
                            p_list.append(name)
                        elif (
                            "(지은이)" in part
                    ...
                    results.append(
                        {
                            ...
                            "Tags": genre,
                            "Genre": genre,
                            "LocalizedSeries": item.get("title", ""),
                            "Count": "",
                            "Rating": rating_detail,
                            "RatingScore": rating_score,
                            "CommunityRating": community_rating,
    ...
    def _search_ridibooks(query, page=1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # [터미널 출력] 전체 도서 개수 로그
                print(
                    f"[RidiBooks API] 응답 데이터 내 전체 책 개수: {len(all_books)}개"
                )
                ...
                for b in paged_books:
                    ...
                    synopsis = (
                        re.sub(
                            r"<[^>]+>", "", str(b.get("desc", b.get("synopsis", "")))
                        ).strip()
                        if b.get("desc") or b.get("synopsis")
                        else ""
                    )

                    cover_obj = b.get("cover", {})
                    cover = (
                        cover_obj.get("xxlarge", "")
                        if isinstance(cover_obj, dict)
                        else str(cover_obj)
                    )
                    ...
                    rating_score = (
                        str(round(r_score * 2, 1)) if r_score is not None else "-"
                    )
                    rating_detail = (
                        f"{rating_score} / 10.0 ({r_count})"
                        if r_score is not None and r_count is not None
                        else "-"
                    )
                    community_rating = (
                        str(round(r_score * 2, 1)) if r_score is not None else ""
                    ...
                    pub_date = b.get("web_title_pub_date", "") or b.get(
                        "publication_date", ""
                    )
                    year, month, day = MetaApiFetcher._parse_date(pub_date)
                    ...
                    results.append(
                        {
                            ...
                            "Summary": synopsis,
                            "Series": title,
                            "Web": f"https://ridibooks.com/books/{b_id}"
                            if b_id
                            else "",
                            "CoverUrl": cover,
                            "Tags": ", ".join(
                            ...
                            "CommunityRating": community_rating,
                            "AgeRating": "19세 이상"
                            if b.get("is_adult_only")
                            else str(b.get("age_limit", "전체 이용가")),
                            "PubDate": pub_date,
                            "Year": year,
                            "Month": month,
                            "Day": day,
                ...
                # [터미널 출력] 최종 파싱 결과 요약
                print(
                    f"[RidiBooks API] 현재 페이지({page}) 파싱된 결과 수: {len(results)}개"
                )
                for idx, res in enumerate(results):
                    print(
                        f"  └ [{idx + 1}] 제목: {res['Title'][:20]}... | 작가: {res['Writer'][:15]} | 출간일: {res['PubDate']}"
                    )
            else:
                # [터미널 출력] 통신 실패 시 로그

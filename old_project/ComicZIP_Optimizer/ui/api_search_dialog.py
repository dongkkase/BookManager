...
from tasks.api_workers import ImageLoadThread, SearchWorker, TranslateWorker
...
class SearchResultWidget(QWidget):
    def __init__(self, data, config, parent=None):
        ...
        self.lbl_title.setStyleSheet(
            f"QLabel {{ font-weight: bold; font-size: {self.config['s14']}px; color: #EAEAEA; background-color: transparent; border: none; outline: none; }}"
        )
        ...
        self.lbl_summary.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum
        )
        self.lbl_summary.setStyleSheet(
            f"QLabel {{ color: #999999; font-size: {self.config['s12']}px; background-color: transparent; border: none; outline: none; }}"
        )
        ...
        def add_meta_item(text, color):
            lbl = QLabel(text)
            lbl.setStyleSheet(
                f"QLabel {{ color: {color}; font-size: {self.config['s11']}px; background-color: transparent; border: none; outline: none; }}"
            )
        ...
        def add_divider():
            lbl = QLabel("|")
            lbl.setStyleSheet(
                f"QLabel {{ color: rgba(255, 255, 255, 0.3); font-size: {self.config['s9']}px; background-color: transparent; border: none; outline: none; }}"
            )
            meta_layout.addWidget(lbl)
        ...
        if rating and rating != "-":
            ...
            star_lbl.setStyleSheet(
                f"QLabel {{ color: #F1C40F; font-size: {self.config['s11']}px; font-weight: bold; background-color: transparent; border: none; outline: none; }}"
            )
    ...
    def _set_default_thumb(self):
        ...
        if os.path.exists(p):
            try:
                ...
                self.lbl_thumb.setStyleSheet(
                    "QLabel { background-color: #333; border-radius: 5px; }"
                )
        else:
            self.lbl_thumb.setStyleSheet(
                "QLabel { background-color: #333; border-radius: 5px; }"
    ...
    def on_image_loaded(self, img_data, url):
        try:
            if getattr(self, "lbl_thumb", None) is None:
                return
...
class ApiSearchDialog(QDialog):
    def __init__(self, api_name, query, h1_text, parent=None, t=None):
        ...
        self.t = t if t else {}
        self.config = (
            parent.main_app.config if parent and hasattr(parent, "main_app") else {}
        )
    ...
    def setup_ui(self):
        ...
        self.lbl_h1 = QLabel(
            f"<h1 style='margin:0; color:#3498DB;'>{self.h1_text}</h1>"
        )
        ...
        self.cb_api.setStyleSheet(
            "padding: 6px; border: 1px solid #555; border-radius: 4px; background-color: #2b2b2b;"
        )
        ...
        self.le_query.setStyleSheet(
            "padding: 7px; border: 1px solid #555; border-radius: 4px; background-color: #2b2b2b;"
        )
        ...
        self.btn_search.setStyleSheet(
            "background-color: #444; color: white; border: 1px solid #555; padding: 7px 20px; border-radius: 4px; font-weight: bold;"
        )
        self.btn_search.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_search.clicked.connect(self.action_manual_search)

        self.lbl_api_warning = QLabel(
            self.t.get("api_key_missing", "환경설정에서 API 키를 입력해주세요.")
        )
        self.lbl_api_warning.setStyleSheet(
            f"color: #E74C3C; font-weight: bold; font-size: {self.config['s12']}px; margin-left: 10px;"
        )
        ...
        left_pane.setStyleSheet(
            "QWidget { background-color: #444; border: 1px solid #444; border-radius: 6px; }"
        )
        ...
        self.lbl_page_info.setStyleSheet(
            "color: #ddd; font-weight: bold; border: none;"
        )
        ...
        self.btn_next_page.setLayoutDirection(
        ...
        self.lbl_result_count = QLabel(
            f"{self.t.get('search_result_prefix', '검색 결과:')} 0{self.t.get('search_result_suffix', '건')}"
        )
        self.lbl_result_count.setStyleSheet(
            f"color: #aaa; font-size: {self.config['s12']}px; margin-top: 5px; border: none; outline: none;"
        )
        ...
        self.scroll_area.setStyleSheet(
            "QScrollArea { border: 1px solid #444; border-radius: 6px; background-color: #444; }"
        )
        ...
        self.lbl_detail_title.setStyleSheet(
            f"font-size: {self.config['s20']}px; font-weight: bold; color: white; padding-bottom: 5px;"
        )
        self.lbl_detail_title.setWordWrap(True)
        self.lbl_detail_title.setTextInteractionFlags(
        ...
        self.btn_translate.setStyleSheet(
            f"background-color: #27AE60; color: white; padding: 5px 15px; border-radius: 4px; font-weight: bold; font-size: {self.config['s11']}px;"
        )
        ...
        self.lbl_cover.setStyleSheet(
            "background-color: transparent; border: none; outline: none; color: #555;"
        )
        ...
        self.detail_star_icon.setPixmap(
            qta.icon("fa5s.star", color="#f1c40f").pixmap(12, 12)
        )
        self.detail_star_icon.setStyleSheet(
            "QLabel { background-color: transparent; border: none; outline: none; }"
        )
        ...
        def _add_bottom_section(title):
            ...
            lbl_v.setStyleSheet(
                f"color: #ddd; font-size: {self.config['s13']}px; line-height: 1.5;"
            )
            lbl_v.setCursor(Qt.CursorShape.IBeamCursor)

            detail_layout.addWidget(lbl_t)
            detail_layout.addWidget(lbl_v)
            return lbl_v
        ...
        self.lbl_cache_notice = QLabel(
            self.t.get(
                "api_cache_notice", "빠른 표시를 위해 검색 결과는 7일간 캐싱됩니다."
            )
        )
        self.lbl_cache_notice.setStyleSheet(
            f"color: #888; font-size: {self.config['s11']}px; border: none;"
        )
        ...
        self.btn_close.setStyleSheet(
            "background-color: #555; color: white; padding: 8px 25px; border-radius: 4px; font-weight: bold;"
        )
        ...
        self.btn_select.setStyleSheet(
            "background-color: #3498DB; color: white; padding: 8px 25px; border-radius: 4px; font-weight: bold;"
        )
    ...
    def perform_search(self):
        ...
        if (
            self.current_api == "Google Books"
            and not self.api_keys.get("google", "").strip()
        ):
            self.lbl_api_warning.show()
            self._clear_results()
            return
        ...
        page_str = self.t.get("api_page_info", "{page} 페이지").format(
    ...
    def _on_search_finished(self, raw_results, actual_query):
        if (
            ...
            and not raw_results
            and actual_query == "RATE_LIMIT"
        ):
            Toast.show(
                self,
                self.t.get(
                    "api_rate_limit",
                    "API 호출 한도 초과입니다. 잠시 후 다시 시도해주세요.",
                ),
            )
        ...
        if isinstance(raw_results, list) and raw_results:
            ...
            raw_results.sort(
                key=lambda x: similar(query_lower, str(x.get("Title", "")).lower()),
                reverse=True,
            )
        ...
        if actual_query and actual_query.lower() != self.current_query.lower():
            res_text += (
                f" <span style='color:#E67E22;'>(번역된 키워드: {actual_query})</span>"
            )
    ...
    def on_item_selected(self):
        ...
        if self.current_api == "리디북스" and self.selected_raw_data.get("b_id"):
            if not self.selected_raw_data.get("Month"):
                ...
                if pub_date:
                    ...
                    if len(parts) >= 1:
                        self.selected_raw_data["Year"] = parts[0]
                    if len(parts) >= 2:
                        self.selected_raw_data["Month"] = str(int(parts[1]))
                    if len(parts) >= 3:
                        self.selected_raw_data["Day"] = str(int(parts[2]))

                    self.search_results[row]["PubDate"] = pub_date
                    self.search_results[row]["Year"] = self.selected_raw_data.get(
                        "Year", ""
                    )
                    self.search_results[row]["Month"] = self.selected_raw_data.get(
                        "Month", ""
                    )
                    self.search_results[row]["Day"] = self.selected_raw_data.get(
                        "Day", ""
                    )
        # ------------------------------------------------------------------
        ...
        self.btn_translate.setIcon(qta.icon("fa5s.language", color="white"))
        self.btn_translate.setStyleSheet(
            "background-color: #27AE60; color: white; padding: 5px 15px; border-radius: 4px; font-weight: bold;"
        )

        if self.current_api in ["Anilist", "Vine", "Google Books"]:
    ...
    def _apply_tag_rules(self, text):
        if not text or not self.tag_rules:
            return text
    ...
    def update_detail_panel(self):
            ...
            if key == "Rating":
                if val and val != "-":
                    ...
                    lbl_widget.setStyleSheet(
                        f"color: #F1C40F; font-size: {self.config['s13']}px; font-weight: bold;"
                    )
                else:
                    self.detail_star_icon.hide()
                    lbl_widget.setStyleSheet(
                        f"color: #ddd; font-size: {self.config['s13']}px;"
                    )
        ...
        if web_link:
            self.lbl_web.setText(
                f"<a href='{web_link}' style='color: #3498DB;'>{web_link}</a>"
            )
        else:
            self.lbl_web.setText("-")
    ...
    def _on_cover_loaded(self, img_data, url):
        try:
            if (
                getattr(self, "lbl_cover", None) is None
                or self.current_cover_url != url
            ):
                return
            if img_data:
                image = QImage.fromData(img_data)
    ...
    def action_translate(self):
        ...
        if self.is_translated:
            self.btn_translate.setText(
                f" {self.t.get('btn_translating', '번역 중...')}"
            )
            self.btn_translate.setIcon(
                qta.icon("fa5s.spinner", spin=True, color="white")
            )
            self.btn_translate.setEnabled(False)

            self.translate_worker = TranslateWorker(
                self.selected_raw_data, getattr(self, "api_keys", {}), self.target_lang
            )
            self.translate_worker.finished_translation.connect(
    ...
    def _on_translation_finished(self, raw_data, translated_data):
        ...
        self.btn_translate.setStyleSheet(
            "background-color: #E67E22; color: white; padding: 5px 15px; border-radius: 4px; font-weight: bold;"
        )
    ...
    def action_apply(self):
        if not self.selected_raw_data:
            return
        result_data = (
            self.translated_data if self.is_translated else self.selected_raw_data
        )

...
class TagWidget(QFrame):
    def __init__(self, text, remove_cb, config):
        ...
        self.config = config
        self.setStyleSheet(
            f"QFrame {{ background-color: #3a7ebf; border-radius: 4px; }} QLabel {{ color: white; padding: 4px 2px 4px 6px; border: none; background: transparent; font-weight: bold; font-size: {self.config['s11']}px; }} QPushButton {{ border: none; background: transparent; color: white; padding: 4px 6px 4px 2px; font-weight: bold; }} QPushButton:hover {{ color: #ffcccc; }}"
        )
...
class TagInputArea(QFrame):
    def __init__(self, i18n_dict, config, on_change_cb=None):
        ...
        self.setStyleSheet(
            "TagInputArea { background-color: #1a1a1a; border: 1px solid #555; border-radius: 4px; }"
        )

        self.flow_layout = FlowLayout(self, margin=10, spacing=8)
        self.line_edit = TagLineEdit(self)
        self.line_edit.setStyleSheet(
            "background: transparent; border: none; color: white; padding-left: 2px;"
        )
        self.line_edit.setMinimumWidth(80)

        self.line_edit.setPlaceholderText(
            self.i18n.get("enter_after_input", "입력 후 Enter...")
        )
    ...
    def _add_tag_ui(self, text):
        ...
        tag_widget = TagWidget(text, self.remove_tag, self.config)

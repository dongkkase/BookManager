...
class SearchableComboBox(QComboBox):
    ...
    def _show_context_menu(self, pos):
        ...
        delete_action = menu.addAction(
            "선택된 항목 저장목록에서 삭제" if is_ko else "Delete from saved list"
        )
...
class DimOverlay(QWidget):
    def __init__(
        self, parent=None, show_spinner=False, text="", play_sound_on_hide=False
    ):
        super().__init__(parent)
        ...
        if self.show_spinner:
            ...
            if anim_path:
                ...
                if orig_size.isValid():
                    self.movie.setScaledSize(
                        QSize(orig_size.width() * 1, orig_size.height() * 1)
                    )
                ...
                self.anim_label.setPixmap(
                    qta.icon("fa5s.spinner", color="#3498DB").pixmap(62, 62)
                )
            ...
            self.text_label.setStyleSheet(
                "color: white; font-size: 14px; font-weight: bold; background: transparent; border: none; margin-top: 15px;"
            )
    ...
    def hideEvent(self, event):
        ...
        if (
            getattr(self, "play_sound_on_hide", False)
            and getattr(self, "_show_time", 0) > 0
        ):

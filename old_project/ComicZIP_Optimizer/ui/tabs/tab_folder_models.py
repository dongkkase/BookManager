...
class CustomHeaderView(QHeaderView):
    ...
    def paintSection(self, painter, rect, logicalIndex):
        ...
        text = model.headerData(
            logicalIndex, self.orientation(), Qt.ItemDataRole.UserRole
        )
        if not text:
            return
        ...
        font = model.headerData(
            logicalIndex, self.orientation(), Qt.ItemDataRole.FontRole
        )
        if font:
        ...
        text_rect = QRect(
            x_offset + 16, rect.y(), rect.width() - 24 - right_margin, rect.height()
        )
    ...
    def mousePressEvent(self, event):
        ...
        if index.isValid() and event.button() == Qt.MouseButton.LeftButton:
            ...
            if row_data.get("is_dup_folder"):
                ...
                btn_rect = QRect(
                    rect.left() + 20 + text_width + 15, rect.top() + 5, 90, 24
                )
...
class ThumbnailDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if row_data.get("is_group"):
            ...
            if missing:
                if len(missing) > 5:
                    missing_str = _("tf_missing_vols_more").format(
                ...
                painter.drawText(
                    option.rect.adjusted(10 + text_width + 10, 0, -10, -5),
                    flags,
                    missing_str,
                )
        ...
        if row_data.get("is_dup_folder"):
            ...
            btn_rect = QRect(
                option.rect.left() + 20 + text_width + 15, option.rect.top() + 5, 90, 24
            )
            ...
            if col_id == "cover":
                ...
                if not pixmap.isNull():
                    ...
                    cropped_pixmap = scaled_pixmap.copy(
                        crop_x, crop_y, target_rect.width(), target_rect.height()
                    )
        ...
        row = index.row()
        target_scale = (
            1.05 if (is_hovered and self.view_mode in ["thumbnail", "tile"]) else 1.0
        )
        ...
        if self.view_mode == "thumbnail":
            ...
            fixed_w = int(
                img_size / 1.414
            )  # 책 커버 표준 비율(1:1.414)로 캔버스 너비 고정
            ...
            painter.drawRoundedRect(
                img_rect.translated(stack_offset * 2, stack_offset * 2), 4, 4
            )
            painter.drawRoundedRect(
                img_rect.translated(stack_offset, stack_offset), 4, 4
            )
            ...
            grad_h = int(nh * 0.4)
            grad_rect = QRect(
                img_rect.x(), img_rect.y() + img_rect.height() - grad_h, nw, grad_h
            )
            gradient = QLinearGradient(
                0, grad_rect.y(), 0, grad_rect.y() + grad_rect.height()
            )
            ...
            rating = (
                row_data.get("rating", "")
                or full_meta.get("rating", "")
                or full_meta.get("Rating", "")
            )
            page_count = (
                row_data.get("page_count", "")
                or full_meta.get("page_count", "")
                or full_meta.get("PageCount", "")
            )
            ...
            writer = (
                row_data.get("writer", "")
                or full_meta.get("writer", "")
                or full_meta.get("Writer", "")
            )
            publisher = (
                row_data.get("publisher", "")
                or full_meta.get("publisher", "")
                or full_meta.get("Publisher", "")
            )
            genre = (
                row_data.get("genre", "")
                or full_meta.get("genre", "")
                or full_meta.get("Genre", "")
            )

            sub1_parts = [
                str(p)
                for p in [writer, publisher, genre]
                if p and str(p).strip() and str(p) != "-"
            ]
            sub1_text = " · ".join(sub1_parts) if sub1_parts else _("info_no_series")

            sub1_elided = fm_sub.elidedText(
                sub1_text, Qt.TextElideMode.ElideRight, max_text_width
            )
            ...
            sub2_parts = []
            if page_count and str(page_count) != "-":
                sub2_parts.append(f"{page_count}p")
            if rating and str(rating) != "-":
                sub2_parts.append(f"★ {rating}")

            sub2_text = " · ".join(sub2_parts)
            if sub2_text:
                sub2_elided = fm_sub.elidedText(
                    sub2_text, Qt.TextElideMode.ElideRight, max_text_width
                )
            ...
            if summary and summary != "-":
                ...
                desc_rect = QRect(
                    text_x, current_y, max_text_width, rect.bottom() - current_y
                )
                flags_desc = (
                    Qt.AlignmentFlag.AlignLeft.value
                    | Qt.AlignmentFlag.AlignTop.value
    ...
    def sizeHint(self, option, index):
        ...
        row_data = index.model()._data[index.row()]
        if (
            row_data.get("is_group")
            or row_data.get("is_dup_folder")
            or row_data.get("is_dup_child")
        ):
            width = (
...
class LibraryTableModel(QAbstractTableModel):
    def __init__(self, data=None, thumb_dir=""):
        ...
        self.ALL_COLUMNS = {
            ...
            "num": _("col_num"),
            "writer": _("col_writer"),
            "series_group": _("col_series_group"),
            "creators": _("col_creators"),
            "publisher": _("col_publisher"),
            "imprint": _("col_imprint"),
            "genre": _("col_genre"),
            "volume_count": _("col_vol_count"),
            "page_count": _("col_page_count"),
            "format": _("col_format"),
            "manga": _("col_manga"),
            "language": _("col_language"),
            "rating": _("col_rating"),
            "age_rating": _("col_age_rating"),
            "publish_date": _("col_pub_date"),
            "summary": _("col_summary"),
            "characters": _("col_characters"),
            "teams": _("col_teams"),
            "locations": _("col_locations"),
            "story_arc": _("col_story_arc"),
            "tags": _("col_tags"),
            "notes": _("col_notes"),
            "web": _("col_web"),
        }
    ...
    def headerData(self, section, orientation, role=Qt.ItemDataRole.DisplayRole):
        ...
        if orientation == Qt.Orientation.Horizontal:
            if section >= len(self.active_columns):
                return None

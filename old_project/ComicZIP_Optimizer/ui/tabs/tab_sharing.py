...
class TabSharing(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_opds_toggle.clicked.connect(
            lambda: self.toggle_server(
                "OPDS", self.opds_port_spin, self.btn_opds_toggle
            )
        )
        ...
        self.btn_opds_copy.clicked.connect(
            lambda: QApplication.clipboard().setText(self.opds_url_label.text())
        )
        ...
        self.btn_webdav_toggle.clicked.connect(
            lambda: self.toggle_server(
                "WebDAV", self.webdav_port_spin, self.btn_webdav_toggle
            )
        )
        ...
        self.btn_webdav_copy.clicked.connect(
            lambda: QApplication.clipboard().setText(self.webdav_url_label.text())
        )
    ...
    def retranslate_ui(self, t, lang):
        self.i18n_t = t
        self.lang = lang
        self.opds_group.setTitle(
            t.get("tab_sharing_opds_title", "OPDS 공유 서버 (Panels 지원)")
        )
        self.webdav_group.setTitle(
            t.get("tab_sharing_webdav_title", "WebDAV 공유 서버 (ComicGlass 지원)")
        )
    ...
    def _update_btn_text(self, btn, protocol):
        ...
        is_running = (
            protocol in self.server_manager.servers
            and self.server_manager.servers[protocol].is_running
        )
    ...
    def update_urls(self):
        ...
        self.opds_url_label.setText(
            f"{scheme}://{ip}:{self.opds_port_spin.value()}/opds"
        )
        if hasattr(self, "webdav_url_label"):
            self.webdav_url_label.setText(
                f"http://{ip}:{self.webdav_port_spin.value()}/"
            )
    ...
    def save_ports(self):
        ...
        if hasattr(self, "webdav_port_spin"):
            ...
            self.config["webdav_username"] = (
                self.webdav_id_input.text().strip() or "user"
            )
            self.config["webdav_password"] = (
                self.webdav_pw_input.text().strip() or "1234"
            )
    ...
    def toggle_server(self, protocol, port_spin, btn_toggle):
        ...
        if is_turning_on:
            ...
            if success:
                ...
                msg_stopped = t.get(
                    "tab_sharing_msg_stopped",
                    "{protocol} 서버가 성공적으로 중지되었습니다.",
                )

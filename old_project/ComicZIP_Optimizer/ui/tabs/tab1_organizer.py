...
from PyQt6.QtGui import QAbstractTextDocumentLayout, QColor, QPixmap, QTextDocument
...
class RichTextDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        ...
        if text and "||" in text:
            ...
            warning_icon = (
                '<span style=\'color: #f1c40f; font-family: "Font Awesome 5 Free Solid", "FontAwesome", sans-serif;\'>&#xf071;</span> '
                if is_spinoff
                else ""
            )
    ...
    def sizeHint(self, option, index):
        ...
        if text and "||" in text:
            ...
            warning_icon = (
                '<span style=\'font-family: "Font Awesome 5 Free Solid", "FontAwesome", sans-serif;\'>&#xf071;</span> '
                if is_spinoff
                else ""
            )
            spacer = "\t\t" if orig_text else ""
            ...
            doc = QTextDocument()
            doc.setHtml(
                f"<span style='white-space: pre;font-weight: bold;'>{main_text}</span>{warning_icon}<span style='white-space: pre;'>{spacer}({orig_text})</span>"
            )
            doc.setDefaultFont(options.font)
            ...
            final_height = (
                max(doc_height, base_hint.height())
                if base_hint.isValid()
                else doc_height
            )
...
class Tab1Organizer(QWidget):
    ...
    def setup_ui(self):
        ...
        self.lbl_empty_org.setStyleSheet(
        ...
        self.tree_org.setSelectionMode(
            QAbstractItemView.SelectionMode.ExtendedSelection
        )
        self.tree_org.setHeaderLabels(["", "", "", ""])

        self.tree_org.header().setSectionResizeMode(
    ...
    def retranslate_ui(self, t, lang):
        self.btn_toggle_expand.setText(
            "↕ 전체 펼치기 / 접기" if lang == "ko" else "↕ Expand / Collapse All"
        )
        ...
        # 헤더 텍스트 우측 정렬
        self.tree_org.headerItem().setTextAlignment(
    ...
    def toggle_expand(self):
        if self.tree_org.topLevelItemCount() == 0:
            return
        ...
        for fp, data in self.org_data.items():
            if data.get("checked", False) and "le_path" in data:
                if data["clean_title"] == "제목없음":
                    data["le_path"].setText(
                        os.path.join(os.path.dirname(fp), "제목없음_수정필요")
                    )
                else:
                    data["le_path"].setText(
                        os.path.join(os.path.dirname(fp), data["clean_title"])
                    )
    ...
    def refresh_list(self):
        ...
        target_ext = (
            f".{self.main_app.config.get('target_format', 'zip')}"
            if self.main_app.config.get("target_format", "none") != "none"
            else ""
        )
        items_to_add = []
        root_widgets_to_set = []
        ...
        for fp, data in self.org_data.items():
            ...
            vol_count_text = (
                f"{len(data['volumes'])} Items"
                if self.main_app.lang == "en"
                else f"{len(data['volumes'])} 권/화"
            )

            root_item.setText(2, vol_count_text)
            root_item.setText(3, f"{data['size_mb']:.1f} MB")

            root_item.setTextAlignment(
                2, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
            root_item.setTextAlignment(
                3, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            ...
            title_path = os.path.join(
                ...
                "제목없음_수정필요"
                if data["clean_title"] == "제목없음"
                else data["clean_title"],
            )

            if "out_path" not in data:
                data["out_path"] = default_path
            le_path.setText(data["out_path"])
            le_path.textChanged.connect(
                lambda text, key=fp: self.update_out_path(key, text)
            )
            data["le_path"] = le_path

            btn_def = QPushButton("기본값" if self.main_app.lang == "ko" else "Default")
            btn_def.clicked.connect(
                lambda _, key=fp, p=default_path: self.set_single_path(key, p)
            )
            btn_tit = QPushButton("책제목" if self.main_app.lang == "ko" else "Title")
            btn_tit.clicked.connect(
                lambda _, key=fp, p=title_path: self.set_single_path(key, p)
            )
            ...
            btn_vol_text = (
                "일괄: 권"
                if lang == "ko"
                else ("一括: 巻" if lang == "ja" else "All: Vol")
            )
            btn_ch_text = (
                "일괄: 화"
                if lang == "ko"
                else ("一括: 話" if lang == "ja" else "All: Ch")
            )

            btn_vol = QPushButton(btn_vol_text)
            btn_vol.clicked.connect(
                lambda _, key=fp: self.batch_change_unit(key, "vol")
            )
            ...
            # 테두리와 높이를 명확히 지정
            le_path.setStyleSheet(
                f"padding: 5px; font-size: {s11}px; border: 1px solid #555; border-radius: 4px; background-color: #3a3a3a; color: white;"
            )
            ...
            for vol in data["volumes"]:
                ...
                final_ext = (
                    ...
                    if target_ext
                        ...
                        Path(vol.get("inner_path", "")).suffix
                        if vol.get("type") == "archive"
                        else ".zip"
                    )
                )
                if not final_ext:
                ...
                if len(path_parts) > 1:
                    ...
                    match = re.search(
                        r"(\d+\s*부|제\s*\d+\s*부|시즌\s*\d+|season\s*\d+|part\s*\d+)",
                        parent_folder,
                        re.IGNORECASE,
                    )
                    prefix = (
                ...
                child.setText(
                    0, f"  ↳ {icon_txt} {new_display}||{orig_name}||{spinoff_flag}"
                )
        ...
        for item, widget in root_widgets_to_set:
        ...
        for i in range(self.tree_org.topLevelItemCount()):
            ...
            should_expand = saved_expanded_states.get(
                fp, getattr(self, "is_expanded", True)
            )
    ...
    def remove_selected(self):
        fps_to_remove = [
            fp for fp, data in self.org_data.items() if data.get("checked", False)
        ]
        for fp in fps_to_remove:
            del self.org_data[fp]
        self.refresh_list()
    ...
    def remove_highlighted(self):
        ...
        if total_rows > 0:
            self.tree_org.topLevelItem(
                min(next_select_row, total_rows - 1)
            ).setSelected(True)
    ...
    def edit_child_filename(self, item):
        ...
        layout.addWidget(
            QLabel("새 파일명을 입력하세요:" if is_ko else "Enter new filename:")
        )
            ...
            if new_name:
                ...
                final_ext = (
                    ...
                    if target_ext
                        ...
                        Path(vol_data.get("inner_path", "")).suffix
                        if vol_data.get("type") == "archive"
                        else ".zip"
                    )
                )
                if not final_ext:
                ...
                orig_name = vol_data.get(
                    "original_basename", os.path.basename(orig_path.replace("\\", "/"))
                )
                prefix = ""

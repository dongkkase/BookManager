...
# [속도/정밀도 개선 핵심 정규식]
RE_TRASH_1 = re.compile(
    r"(?i)\b(1080p|720p|480p|1440p|4k|2k|x264|x265|\d{3,4}x\d{3,4})\b"
)
...
RE_RANGE_TILDE = re.compile(
    r"(\d+)\s*(?:권|화|장|편|부|vol|ch|ep)?\s*[~～〜]\s*(\d+)\s*(?:권|화|장|편|부|vol|ch|ep)?",
    re.IGNORECASE,
)

# 대시(-)는 13권-56(에피소드)과 13-56화(범위)를 엄격히 구분하기 위해 우측이나 양쪽에 명시적 단위가 있을 때만 범위로 인정
RE_RANGE_DASH_1 = re.compile(
    r"(\d+)\s*-\s*(\d+)\s*(권|화|장|편|부|vol|ch|ep)", re.IGNORECASE
)
RE_RANGE_DASH_2 = re.compile(
    r"(\d+)\s*(권|화|장|편|부|vol|ch|ep)\s*-\s*(\d+)\s*\2", re.IGNORECASE
)
...
class DupScanThread(QThread):
    ...
    def run(self):
        ...
        for folder in self.dup_folders:
            ...
            db_paths = (
                {record["full_path"]: record for record in target_records}
                if target_records
                else {}
            )
            new_records = []
            ...
            if len(db_paths) > len(current_physical_paths) or any(
                p not in current_physical_paths for p in db_paths
            ):
                try:
                    db.clear_dup_cache()
                except Exception:
...
class IndexSyncThread(QThread):
    ...
    def run(self):
        ...
        for folder in self.dup_folders:
            ...
            target_records = db.get_target_index(folder)
            db_paths = (
                {record["full_path"] for record in target_records}
                if target_records
                else set()
            )
        ...
        if b_folder_changed:
            try:
                db.clear_dup_cache()
                print(
                    "[LOG] 백그라운드 인덱스 동기화: 대상 폴더 변경 감지로 매칭 캐시 초기화됨."
                )
            except Exception:
                pass
...
class DupMatchThread(QThread):
    ...
    def extract_series_and_nums(self, name):
        ...
        if bundle_match:
            return (
                core_title,
                [float(bundle_match.group(1)), float(bundle_match.group(2))],
                True,
            )
        ...
        ko_single = re.search(
            r"(\d+(?:\.\d+)?)\s*(?:권|화|장|편|부)", name, re.IGNORECASE
        )
        if ko_single:
            return core_title, [float(ko_single.group(1))], False

        en_single = re.search(
            r"(?:v|vol|c|ch|chapter|제)\.?\s*(\d+(?:\.\d+)?)", name, re.IGNORECASE
        )
        if en_single:
    ...
    def get_char_and_bigrams(self, text):
        ...
        for word in self.STOPWORDS:
            if re.match(r"^[a-z]+$", word):
                ...
                text = re.sub(
                    r"(?<![가-힣a-z])" + re.escape(word) + r"(?![가-힣a-z])", "", text
                )
    ...
    def run(self):
        ...
        msg_match_all = (
            get_i18n()
            .get(lang, get_i18n()["ko"])
            .get("msg_folder_match_all", "(Folder Full Match)")
        )
        ...
        for idx, b_file in enumerate(self.b_cache):
            ...
            if bp not in b_folders:
                ...
                f_core, _, _ = (
                    self.extract_series_and_nums(folder_name)
                    if folder_name
                    else ("", [], False)
                )
        ...
        for i, (a_file, a_core, a_nums, a_is_bundle, a_char, a_bigrams) in enumerate(
            a_data
        ):
            if self.is_cancelled:
                return
            ...
            for bp, b_folder in b_folders.items():
                ...
                for b_file in b_folder["files"]:
                    if self.is_cancelled:
                        return
                    b_full_path = os.path.normcase(
                        os.path.normpath(b_file.get("full_path", ""))
                    )
                    if a_path == b_full_path:
                        continue
...
class MemoryExtractThread(QThread):
            ...
            try:
                if ext in [".zip", ".cbz"]:
                    with zipfile.ZipFile(filepath, "r") as zf:
                        ...
                        if needs_meta:
                            xml_name = next(
                                (f for f in namelist if f.lower() == "comicinfo.xml"),
                                None,
                            )
                            if xml_name:
                                xml_data = zf.read(xml_name).decode(
                                    "utf-8", errors="ignore"
                                )
                    ...
                    for line in lines:
                        if line.startswith("Path = "):
                            ...
                            if fname.lower() == "comicinfo.xml":
                                has_xml = True
                            elif needs_img and fname.lower().endswith(
                                (".jpg", ".jpeg", ".png", ".webp", ".bmp")
                            ):
                    ...
                    if needs_meta and has_xml:
                        ...
                        if res_x.returncode == 0:
                            meta_dict = self._parse_xml(
                                res_x.stdout.decode("utf-8", errors="ignore")
                            )
    ...
    def run(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(self._process_task, task) for task in self.current_tasks
            ]
    ...
    def _parse_xml(self, xml_data):
        ...
        try:
            ...
            meta["rating"] = root.findtext("CommunityRating") or root.findtext(
                "Rating", ""
            )
...
class FolderScanThread(QThread):
    ...
    def __init__(
        self, folder_path, include_sub, target_exts, thumb_dir, force_update=False
    ):
        super().__init__()
                        ...
                        if entry.is_dir(follow_symlinks=False):
                            ...
                            if name.lower().endswith(self.target_exts):
                                ...
                                if cached and not self.force_update:
                                    ...
                                    if abs(float(cached_mtime) - float(mtime)) < 2.0:
                                        ...
                                        full_meta = {
                                            ...
                                            "volume_count": cached[15]
                                            if len(cached) > 15
                                            else "",
                                            "page_count": cached[16]
                                            if len(cached) > 16
                                            else "",
                                            "format": cached[17]
                                            if len(cached) > 17
                                            ...
                                            "age_rating": cached[21]
                                            if len(cached) > 21
                                            else "",
                                            "publish_date": cached[22]
                                            if len(cached) > 22
                                            else "",
                                            "summary": cached[23]
                                            if len(cached) > 23
                                            else "",
                                            "characters": cached[24]
                                            if len(cached) > 24
                                            else "",
                                            "teams": cached[25]
                                            if len(cached) > 25
                                ...
                                new_hash = hashlib.md5(
                                    f"{full_path}_{mtime}".encode()
                                ).hexdigest()
                                old_hash = hashlib.md5(
                                    f"{entry.path}_{mtime}".encode()
                                ).hexdigest()
                                ...
                                os.path.join(self.thumb_dir, thumb_filename)
                                ...
                                row_dict = {
                                    ...
                                    "raw_mtime": mtime,
                                    "raw_ctime": ctime,
                                    "ctime": datetime.fromtimestamp(ctime).strftime(
                                        "%Y-%m-%d %H:%M"
                                    ),
    ...
    def run(self):
        ...
        if self.dup_folders:
            ...
            for row in self.file_data_cache:
                ...
                name = row.get("name", "")
                db_series = row.get("series", "") or row.get("full_meta", {}).get(
                    "series", ""
                )

                # 이미 라이브러리에 포함된 경로면 중복 처리 방지
                if not any(
                    fp.startswith(os.path.normpath(d)) for d in self.dup_folders
                ):
                    process_record(fp, name, db_series)

...
from PyQt6.QtGui import QAction, QImage, QIntValidator, QPainter, QPainterPath, QPixmap
...
class Tab2Renamer(QWidget):
    ...
    def update_icons(self, is_dark):
        ...
        if not os.path.exists(draganddrop_path):
            self.icon_empty_arch.setPixmap(
    ...
    def setup_ui(self):
        ...
        self.lbl_cover_img.setSizePolicy(
        ...
        self.lbl_inner_img.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        ...
        left_layout.addWidget(
            self.lbl_cover_img, alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        left_layout.addWidget(
            self.lbl_inner_img, alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        self.lbl_empty_arch.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s16']}px; font-weight: bold;"
        )
        ...
        self.table_archives.setSelectionBehavior(
            QAbstractItemView.SelectionBehavior.SelectRows
        )
        self.table_archives.setEditTriggers(
        ...
        self.table_inner.parent_tab = (
            self  # 테이블이 이 탭의 함수를 호출할 수 있도록 참조 전달
        )
    ...
    def show_context_menu(self, pos):
        ...
        action_open_dir = QAction(
            qta.icon("fa5s.folder-open"), t.get("action_find_path", "경로 찾기"), self
        )
        action_rename = QAction(
            qta.icon("fa5s.edit"), t.get("action_rename_file", "파일 이름 변경"), self
        )
        action_reload = QAction(
            qta.icon("fa5s.sync-alt"),
            t.get("action_reload_file", "다시 불러오기"),
            self,
        )
    ...
    def _rename_archive_file(self, old_filepath, row):
        ...
        if not os.path.exists(old_filepath):
            QMessageBox.warning(
                self,
                t.get("dlg_err", "오류"),
                t.get("msg_file_not_exist", "파일이 존재하지 않습니다."),
            )
            return
        ...
        if ok and new_name and new_name != old_name:
            ...
            if os.path.exists(new_filepath):
                QMessageBox.warning(
                    self,
                    t.get("dlg_err", "오류"),
                    t.get("msg_rename_dup", "동일한 파일이 존재합니다."),
                )
                return
            ...
            try:
                ...
                for i in range(self.table_archives.rowCount()):
                    if (
                        self.table_archives.item(i, 0).data(Qt.ItemDataRole.UserRole)
                        == new_filepath
                    ):
                ...
                QMessageBox.critical(
                    self,
                    t.get("msg_perm_err_title", "권한 오류"),
                    t.get("msg_perm_err_desc", "사용 중입니다."),
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
                    t.get("dlg_err", "오류"),
                    t.get("msg_err_rename_fail", "실패: {0}").format(str(e)),
                )
        ...
        if filepath in self.archive_data:
            ...
            if os.path.exists(filepath):
                ...
                if success:
                    ...
                    Toast.show(
                        self.main_app, t.get("msg_reload_success", "다시 불러왔습니다.")
                    )
                elif success == "nested":
                    Toast.show(
                        self.main_app,
                        t.get("msg_reload_nested", "내부 압축 파일 포함됨."),
                    )
                else:
                    Toast.show(
                        self.main_app, t.get("msg_reload_fail", "불러오기 실패.")
                    )
            else:
    ...
    def retranslate_ui(self, t, lang):
        ...
        self.table_inner.setHorizontalHeaderLabels(
            [
                t["col_old"],
                t["col_new"],
                t["col_fsize"],
                t.get("col_order", "순서 변경"),
            ]
        )
        self.lbl_total_count.setText(
            t["total_files"].format(count=len(self.archive_data))
        )
    ...
    def on_table_item_changed(self, item):
        ...
        if item.column() == 0:
            ...
            if fp and fp in self.archive_data:
                self.archive_data[fp]["checked"] = (
                    item.checkState() == Qt.CheckState.Checked
                )

                # 🌟 [추가] 개별 파일의 체크박스를 끄고 켤 때, 상단의 '전체 선택' 일괄 버튼도 실시간 동기화
                self.all_checked = all(
                    d.get("checked", False) for d in self.archive_data.values()
                )
                self.update_action_buttons()
    ...
    def refresh_list(self):
        if not self.archive_data:
            ...
            self.render_image("inner", None, None)
            self.lbl_total_count.setText(
                self.main_app.i18n[self.main_app.lang]["total_files"].format(count=0)
            )
        ...
        self.cap_all_checked = any(
            d.get("cap_opt", False) for d in self.archive_data.values()
        )
        self.exif_all_checked = any(
            d.get("exif_opt", True) for d in self.archive_data.values()
        )
        self.update_action_buttons()
        ...
        for row, (fp, data) in enumerate(self.archive_data.items()):
            ...
            chk_item.setCheckState(
                Qt.CheckState.Checked if data["checked"] else Qt.CheckState.Unchecked
            )
                ...
                def on_toggled(state, fp=filepath, key=opt_key):
                    if fp in self.archive_data:
                        ...
                        if key == "cap_opt":
                            self.cap_all_checked = any(
                                d.get("cap_opt", False)
                                for d in self.archive_data.values()
                            )
                        elif key == "exif_opt":
                            self.exif_all_checked = any(
                                d.get("exif_opt", True)
            ...
            self.table_archives.setCellWidget(
                row,
                3,
                create_centered_checkbox(data.get("cap_opt", False), fp, "cap_opt"),
            )
            self.table_archives.setCellWidget(
                row,
                4,
                create_centered_checkbox(data.get("exif_opt", True), fp, "exif_opt"),
            )

        self.table_archives.blockSignals(False)
        self.table_archives.setUpdatesEnabled(True)
        self.lbl_total_count.setText(
            self.main_app.i18n[self.main_app.lang]["total_files"].format(
                count=len(self.archive_data)
            )
        )

    def update_inner_preview_list(self):
        if (
            not self.current_archive_path
            or self.current_archive_path not in self.archive_data
        ):
            return
        ...
        for idx, e in enumerate(entries):
            ...
            if keep_name:
                new = os.path.splitext(os.path.basename(old))[0] + ext
            elif pat_idx == 1:
                new = f"Cover{ext}" if idx == 0 else f"Page_{n:0{pad}d}{ext}"
            elif pat_idx == 2:
                new = f"{stem.replace(' ', '_')}_{n:0{pad}d}{ext}"
            elif pat_idx == 3:
                new = (
                    f"{stem.replace(' ', '_')}_Cover{ext}"
                    if idx == 0
                    else f"{stem.replace(' ', '_')}_Page_{n:0{pad}d}{ext}"
                )
            elif pat_idx == 4:
                new = f"{cust_txt.strip() or 'Custom'}_{n:0{pad}d}{ext}"
            else:
                new = f"{n:0{pad}d}{ext}"
            ...
            btn_bottom.clicked.connect(
                lambda _, r=idx: self.move_inner_item(r, len(entries) - 1)
            )
            if idx == len(entries) - 1:
    ...
    def _process_archive_select(self):
        ...
        if not selected:
            return
        fp = self.table_archives.item(selected[0].row(), 0).data(
            Qt.ItemDataRole.UserRole
        )
        if not fp or fp not in self.archive_data:
            return
        ...
        cover = next(
            (
                e
                for e in entries
                if os.path.basename(e["filename"]).lower().startswith("cover")
                and Path(e["filename"]).suffix.lower() in img_exts
            ),
            None,
        )
        if cover:
        ...
        target = next(
            (e for e in entries if Path(e["filename"]).suffix.lower() in img_exts), None
        )
    ...
    def _process_inner_select(self):
        ...
        if not selected or not getattr(self, "current_archive_path", None):
            return
        orig_fp = self.table_inner.item(selected[0].row(), 0).data(
            Qt.ItemDataRole.UserRole
        )
    ...
    def render_image(self, target_id, arc_path, img_data):
        ...
        label_widget = (
            self.lbl_cover_img if target_id == "cover" else self.lbl_inner_img
        )
        cw = 180
        ...
        if not img_data:
            label_widget.setText(
                self.main_app.i18n[self.main_app.lang]["no_preview"]
                if target_id == "cover"
                else self.main_app.i18n[self.main_app.lang]["no_image"]
            )
            return
    ...
    def remove_selected(self):
        ...
        fps_to_remove = [
            fp for fp, data in self.archive_data.items() if data.get("checked", False)
        ]
    ...
    def remove_highlighted(self):
        ...
        if total_rows > 0:
            self.table_archives.selectRow(min(next_select_row, total_rows - 1))
        else:
            self.table_inner.setRowCount(0)
            self.render_image("cover", None, None)
            self.render_image("inner", None, None)
    ...
    def load_archive_info(self, filepath, batch_mode=False):
        try:
            ...
            if ext in [".zip", ".cbz"]:
                ...
                if not os.path.exists(self.main_app.seven_zip_path):
                    return False
                task = FileLoadTask(
                    [],
                    self.main_app.seven_zip_path,
                ...
                entries = sorted(
                    task.get_7z_entries(filepath),
                    key=lambda x: natural_keys(x["filename"]),
                )

            image_exts = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif"}
            img_entries = [
                e for e in entries if Path(e["filename"]).suffix.lower() in image_exts
            ]
            if not img_entries:
                return False
            ...
            if any(Path(e["filename"]).suffix.lower() in nested_exts for e in entries):
                if not batch_mode:
                    msg = f"[{os.path.basename(filepath)}]\n" + self.main_app.i18n[
                        self.main_app.lang
                    ].get("msg_reload_nested", "")
                    QMessageBox.warning(
                        self,
                        self.main_app.i18n[self.main_app.lang].get(
                            "dlg_warn", "Warning"
                        ),
                        msg,
                    )
    ...
    def move_inner_item(self, from_idx, to_idx):
        ...
        if (
            from_idx < 0
            or from_idx >= len(entries)
            or to_idx < 0
            or to_idx >= len(entries)
        ):
            return
    ...
    def update_action_buttons(self):
        ...
        self.btn_sel_all.setText(
            ("☑ " if is_all else "☐ ") + t.get("btn_sel_all", "전체 선택")
        )
        ...
        self.btn_cap_all.setText(
            f"{'☑ ' if is_cap else '☐ '}{t.get('btn_cap_all', '이미지 압축 일괄')} ({qual}%)"
        )
        ...
        self.btn_exif_all.setText(
            ("☑ " if is_exif else "☐ ") + t.get("btn_exif_all", "EXIF 제거 일괄")
        )

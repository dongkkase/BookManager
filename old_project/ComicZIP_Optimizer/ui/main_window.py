...
from config import CURRENT_VERSION, TOOL_7Z, get_resource_path, load_config, save_config
...
from tasks.load_task import FileLoadTask, OrganizerLoadTask
from tasks.organize_task import OrganizerProcessTask
from tasks.rename_task import RenameTask
from tasks.update_task import ReleaseNotesTask, VersionCheckTask
...
class RenamerApp(QMainWindow):
    def __init__(self):
        ...
        self.latest_version_url = (
            "https://github.com/dongkkase/ComicZIP_Optimizer/releases"
        )
    ...
    def on_release_notes_loaded(self, release_list):
        ...
        if not release_list:
            error_lbl = QLabel(
                self.i18n[self.lang].get(
                    "msg_release_load_fail", "Failed to load release notes."
                )
            )
            error_lbl.setStyleSheet(
                f"color: #aaaaaa; font-family: {self.config.get('font_family_str', 'Jua')};"
            )
        ...
        for item in release_list:
            ...
            title_lbl = QLabel(
                f"📦 {item.get('name', '')} <span style='color:#aaaaaa; font-size:13px;'>({item.get('date', '')})</span>"
            )
            title_lbl.setTextFormat(Qt.TextFormat.RichText)
            title_lbl.setStyleSheet(
                f"color: #3498DB; font-size: 20px; font-weight: bold; border: none; font-family: {self.config.get('font_family_str', 'Jua')};"
            )
            ...
            body_lbl.setStyleSheet(
                f"color: #e0e0e0; font-size: 14px; border: none; font-family: {self.config.get('font_family_str', 'Jua')}; line-height: 1.5;"
            )
    ...
    def update_version_button_ui(self):
        if self.latest_version_found:
            update_msg = (
                self.i18n[self.lang]
                .get("msg_update_available", "")
                .format(CURRENT_VERSION, self.latest_version_found)
            )
    ...
    def start_auto_update(self):
        ...
        self.lbl_status.setText(
            self.i18n[self.lang].get("msg_prep_update", "Preparing update...")
        )
    ...
    def setup_ui(self):
        ...
        self.scroll_release.setStyleSheet(
        ...
        self.lock_overlay = DimOverlay(
            self.tabs, show_spinner=True, text="작업을 진행 중입니다..."
        )
    ...
    def apply_dark_theme(self):
        ...
        if hasattr(self.tab1, "update_icons"):
            self.tab1.update_icons(True)
        if hasattr(self.tab2, "update_icons"):
            self.tab2.update_icons(True)
        if hasattr(self.tab3, "update_icons"):
            self.tab3.update_icons(True)

    def update_server_status_icon(self):
        if not hasattr(self, "tab_sharing") or not hasattr(self, "lbl_server_status"):
            return

        active_servers = list(self.tab_sharing.server_manager.servers.keys())

        if active_servers:
            self.lbl_server_status.setPixmap(
                qta.icon("fa5s.broadcast-tower", color="#2ECC71").pixmap(14, 14)
            )
            ...
            tooltip_txt = (
                ...
                if self.lang == "ko"
                    ...
                    f"{tooltips} サーバー稼働中"
                    if self.lang == "ja"
                    else f"{tooltips} Server Running"
                )
            )
    ...
    def apply_language(self):
        ...
        self.tabs.setTabText(
            ...
            t.get(
                ...
                "공유 서버"
                if self.lang == "ko"
                else ("Sharing" if self.lang == "en" else "共有サーバー"),
            ),
        )
        ...
        if hasattr(self.tab1, "retranslate_ui"):
            self.tab1.retranslate_ui(t, self.lang)
        if hasattr(self.tab2, "retranslate_ui"):
            self.tab2.retranslate_ui(t, self.lang)
        if hasattr(self.tab3, "retranslate_ui"):
            self.tab3.retranslate_ui(t, self.lang)
        if hasattr(self.tab_sharing, "retranslate_ui"):
            self.tab_sharing.retranslate_ui(t, self.lang)

        if self.btn_run.objectName() == "actionBtn":
            self.btn_run.setText(f" {t['run_btn']}")
        else:
            self.btn_run.setText(f" {t['cancel_btn']}")

        if not self.lbl_status.text() or self.lbl_status.text() in [
            self.i18n["ko"]["status_wait"],
            self.i18n["en"]["status_wait"],
            self.i18n["ja"]["status_wait"],
        ]:
    ...
    def toggle_ui_elements(self, is_processing):
        ...
        if is_processing:
            if hasattr(self, "lock_overlay"):
                self.lock_overlay.text = self.i18n[self.lang].get(
    ...
    def open_settings(self):
        ...
        if dlg.exec() == int(QDialog.DialogCode.Accepted):
            ...
            format_changed = new_data.get("target_format") != self.config.get(
                "target_format"
            )
            webp_conv_changed = new_data.get("webp_conversion") != self.config.get(
                "webp_conversion"
            )
            webp_qual_changed = new_data.get("img_quality") != self.config.get(
                "img_quality"
            )
            ...
            if format_changed or webp_conv_changed or webp_qual_changed:
                ...
                msg = self.i18n[lang].get(
                    "msg_format_cleared", "All task lists have been cleared."
                )
                Toast.show(self, msg)
            ...
            if old_lang != new_lang or old_font != new_font or old_scale != new_scale:
                msg = self.i18n[new_lang].get(
                    "msg_restart_desc", "Settings changed. Restart?"
                )
    ...
    def add_folder(self):
        folder = QFileDialog.getExistingDirectory(
            self, "Select Directory", options=QFileDialog.Option.DontUseNativeDialog
        )
        if folder:
    ...
    def add_file(self):
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "Select Archives",
            "",
            "Archive files (*.zip *.cbz *.cbr *.7z *.rar)",
    ...
    def toggle_all_checkboxes(self):
        ...
        if hasattr(current, "toggle_all_checkboxes"):
            current.toggle_all_checkboxes()

    def remove_selected(self):
        current = self.tabs.currentWidget()
        if hasattr(current, "remove_selected"):
            current.remove_selected()

    def clear_list(self):
        current = self.tabs.currentWidget()
        if hasattr(current, "clear_list"):
            current.clear_list()
    ...
    def process_paths(self, paths, is_auto_transfer=False):
        ...
        if self.tabs.currentIndex() == 3:
            ...
            if files_dropped and not is_auto_transfer:
                msg = self.i18n[self.lang].get(
                    "msg_drop_folders_desc", "Add entire folder?"
                )
                title = self.i18n[self.lang].get(
            ...
            for d in folders_dropped:
                if d not in final_paths:
                    final_paths.append(d)

            if final_paths:
                self.tab3.load_paths(final_paths)
            return
        ...
        self.progress_bar.setValue(0)
        self.lbl_status.setText(
            self.i18n[self.lang].get("msg_loading_list", "Loading files...")
        )

        if self.tabs.currentIndex() == 1:
            task = OrganizerLoadTask(
                paths, self.seven_zip_path, self.lang, self.signals
            )
        else:
    ...
    def on_organizer_loaded(self, new_data, skipped_files):
        ...
        if skipped_files:
            msg = self.i18n[self.lang].get(
                "msg_org_skip_no_folder", "Skipped files:\n"
            ) + "\n".join(skipped_files[:5])
            if len(skipped_files) > 5:
                msg += (
                    self.i18n[self.lang]
                    .get("msg_more_files", "\n...more")
                    .format(len(skipped_files) - 5)
                )
            QMessageBox.information(
                self, self.i18n[self.lang].get("msg_notice", "Notice"), msg
            )
    ...
    def on_renamer_loaded(self, new_data, nested_files, unsupported_files):
        ...
        if unsupported_files:
            msg = self.i18n[self.lang].get(
                "msg_unsupported_format", "Unsupported:\n"
            ) + "\n".join(unsupported_files[:5])
            if len(unsupported_files) > 5:
                msg += (
                    self.i18n[self.lang]
                    .get("msg_more_files", "\n...more")
                    .format(len(unsupported_files) - 5)
                )
            QMessageBox.warning(
                self, self.i18n[self.lang].get("dlg_warn", "Warning"), msg
            )
        if nested_files:
            msg = self.i18n[self.lang].get(
                "msg_nested_archive", "Nested archives:\n"
            ) + "\n".join(nested_files[:5])
            if len(nested_files) > 5:
                msg += (
                    self.i18n[self.lang]
                    .get("msg_more_files", "\n...more")
                    .format(len(nested_files) - 5)
                )
            QMessageBox.warning(
                self, self.i18n[self.lang].get("dlg_warn", "Warning"), msg
            )
    ...
    def start_process(self):
        ...
        if self.tabs.currentIndex() == 1:
            ...
            if not targets:
                QMessageBox.warning(
                    self,
                    self.i18n[self.lang].get("dlg_warn", "Warning"),
                    self.i18n[self.lang].get("msg_no_targets"),
                )
                return
    ...
    def cancel_process(self):
        ...
        if (
            hasattr(self, "active_task")
            and self.active_task
            and hasattr(self.active_task, "cancel")
        ):
    ...
    def finish_process_org(self, stats, new_fps, was_cancelled):
        ...
        if was_cancelled:
            ...
            QMessageBox.warning(
                self,
                "Cancelled",
                self.i18n[self.lang].get("msg_cancelled", "Process cancelled by user."),
            )
            log_dlg = LogDialog(
                self, stats, self.i18n[self.lang], show_continue_btn=False
            )
            ...
            msg_str = (
                self.i18n[self.lang]
                .get("msg_job_done", "Done!")
                .format(len(stats["success"]), len(stats["skip"]), len(stats["error"]))
            )
    ...
    def finish_process_rename(self, stats, new_archive_data, was_cancelled):
        ...
        for old_fp, val in new_archive_data.items():
            ...
            # 리스트 UI 업데이트 로직
            if old_fp != new_fp:
                if old_fp in self.tab2.archive_data:
                    del self.tab2.archive_data[old_fp]
        ...
        if (
            getattr(self.tab2, "current_archive_path", None)
            and self.tab2.current_archive_path in self.tab2.archive_data
        ):
            for row in range(self.tab2.table_archives.rowCount()):
                if (
                    self.tab2.table_archives.item(row, 0).data(Qt.ItemDataRole.UserRole)
                    == self.tab2.current_archive_path
                ):
        ...
        if was_cancelled:
            ...
            log_dlg = LogDialog(
                ...
                self.i18n[self.lang],
                show_continue_btn=show_cont,
                continue_key="btn_continue_tab3",
            )
            log_dlg.setStyleSheet(self.styleSheet())
    ...
    def closeEvent(self, event):
        is_tab3_running = (
            hasattr(self, "tab3")
            and hasattr(self.tab3, "save_worker")
            and self.tab3.save_worker
            and self.tab3.save_worker.isRunning()
        )
        ...
        if self.is_processing or is_tab3_running:
            ...
            msg = self.i18n[self.lang].get(
                "msg_exit_body",
                "현재 작업이 진행 중입니다. 정말로 프로그램을 종료하시겠습니까?\n(진행 중인 작업은 강제 중단되며 파일이 손상될 수 있습니다.)",
            )

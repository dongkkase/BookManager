    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if index.column() == 1:  # New Name
                ...
                if row[2] == "OK" or row[2] == "정상" or row[2] == "正常":
                    return QColor("#2ECC71")
                else:
                    return QColor("#E74C3C")  # Conflict / Invalid
        return None
...
# --- [마이그레이션 대비] 비즈니스 로직 순수 함수화 ---
# UI 종속성(QThread) 없이 파일 이름 변경 로직만 처리하는 함수입니다.
# Node.js IPC 핸들러 로직으로 쉽게 전환할 수 있습니다.
def process_preview_logic(file_paths, rule_data, i18n, is_cancelled_fn):
    import os
    ...
    status_ok = i18n.get("tf_status_ok", "정상")
    status_conflict = i18n.get("tf_status_conflict", "중복")
    status_invalid = i18n.get("tf_status_invalid", "불가")

    old_str = rule_data.get("old_str", "")
    new_str = rule_data.get("new_str", "")
    use_regex = rule_data.get("use_regex", False)
    case_sens = rule_data.get("case_sens", False)

    use_num = rule_data.get("use_num", False)
    num_start = rule_data.get("num_start", 1)
    num_digits = rule_data.get("num_digits", 3)
    num_pos = rule_data.get("num_pos", 0)

    use_padding = rule_data.get("use_padding", False)
    pad_digits = rule_data.get("pad_digits", 3)
    ...
    for i, old_path in enumerate(file_paths):
        ...
        if ext and (
            (old_str and ext.lower() in old_str.lower())
            or (new_str and ext.lower() in new_str.lower())
        ):
            curr_name = old_name
            curr_ext = ""
        else:
        ...
        try:
            ...
            if old_str:
                if use_regex:
                    ...
                    if not has_wildcard:
                        ...
                        curr_name = re.sub(
                            escaped_old, lambda m: new_str, curr_name, flags=flags
                        )
                    else:
                        pattern_str = ""
                        var_map = {}
            ...
            if use_padding:
                ...
                def pad_match(m):
                    val_int = int(m.group(0))
                    return f"{val_int:0{pad_digits}d}"
            ...
            if use_num:
                ...
                if num_pos == 0:
                    curr_name = f"{num_str}_{curr_name}"
                else:
                    curr_name = f"{curr_name}_{num_str}"
...
class PreviewWorker(QThread):
    ...
    def run(self):
        result_data = process_preview_logic(
            self.file_paths, self.rule_data, self.i18n, lambda: self.is_cancelled
        )
...
class MultiRenameDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.chk_folder_name = QCheckBox(
            self.i18n.get("tf_use_folder_name", "폴더명으로 이름 바꾸기")
        )
        ...
        self.chk_pad.stateChanged.connect(
            lambda state: self.sp_pad.setEnabled(state == Qt.CheckState.Checked.value)
        )
        ...
        self.cb_pos.addItems(
            [
                self.i18n.get("tf_pos_front", "앞(Front)"),
                self.i18n.get("tf_pos_back", "뒤(Back)"),
            ]
        )

        self.num_widgets = [self.sp_start, self.sp_digits, self.cb_pos]
        for w in self.num_widgets:
            w.setEnabled(False)
        self.chk_num.stateChanged.connect(
            lambda state: [
                w.setEnabled(state == Qt.CheckState.Checked.value)
                for w in self.num_widgets
            ]
        )
        ...
        headers = [
            self.i18n.get("tf_col_old_name", "이전 파일 이름"),
        ...
        self.table_view.horizontalHeader().setStyleSheet(
            f"font-size: {self.config['s11']}px;"
        )
    ...
    def execute_rename(self):
        ...
        for row in self.table_model._data:
            ...
            if (
                status == self.i18n.get("tf_status_ok", "정상") or status == "OK"
    ...
    def on_rename_finished(self, success_map, errors):
        ...
        if errors:
            QMessageBox.warning(
                self,
                self.i18n.get("msg_notice", "알림"),
                self.i18n.get("msg_multi_rename_errors").format(
                    len(success_map), "\n".join(errors[:10])
                ),
            )
        else:
            ...
            Toast.show(
                self.parent(),
                self.i18n.get("msg_multi_rename_done").format(len(success_map)),
            )
        self.accept()
...
class LogDialog(QDialog):
    def __init__(
        ...
        continue_key="btn_continue_tab2",
    ):
        super().__init__(parent)
        ...
        lbl_summary = QLabel(
            f"Success: {len(stats['success'])}  |  Skip: {len(stats['skip'])}  |  Error: {len(stats['error'])}"
        )
        # 하드코딩된 font-size를 배율이 적용된 s14로 변경
        lbl_summary.setStyleSheet(
            f"font-weight: bold; font-size: {s14}px; margin-bottom: 10px; color: #ffffff;"
        )
        ...
        self.text_edit.setStyleSheet(
            f"background-color: #1a1a1a; color: #e0e0e0; font-family: Consolas, monospace; font-size: {s11}px; padding: 10px; border: none; border-radius: 4px;"
        )
        ...
        if show_continue_btn:
            ...
            btn_cont.setStyleSheet(
                "background-color: #27AE60; color: white; font-weight: bold; padding: 8px 15px; border-radius: 4px; border: none;"
            )
        ...
        btn_close.setStyleSheet(
            "background-color: #555; color: white; font-weight: bold; padding: 8px 15px; border-radius: 4px; border: none;"
        )
...
class SettingsDialog(QDialog):
    def __init__(self, parent, config, format_keys, i18n):
        ...
        self.chk_pass_skip_meta = QCheckBox(
            self.i18n.get(
        ...
        from config import get_executable_dir, get_resource_path
        ...
        form_layout.addRow(
            self.i18n.get("font_family_lbl", "글꼴:"), self.cb_font_family
        )
        ...
        if config["lang"] == "ko":
            th_lbl_txt = f"{self.slider_threads.value()} 코어"
        elif config["lang"] == "ja":
            th_lbl_txt = f"{self.slider_threads.value()} コア"
        else:
            th_lbl_txt = f"{self.slider_threads.value()} Cores"
        ...
        lbl_threads_desc.setStyleSheet(
            "color: #E74C3C; font-size: {self.config['s11']}px; margin-top: 5px;"
        )
        ...
        lbl_flatten_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px; margin-left: 25px;"
        )
        ...
        lbl_qual_title = QLabel(
            self.i18n.get("common_quality", "이미지 압축 품질 (Quality) :")
        )
        ...
        self.lbl_qual_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px; margin-left: 25px; margin-bottom: 10px;"
        )
        opt_layout.addWidget(self.lbl_qual_desc)

        opt_layout.addSpacing(10)
        self.chk_webp = QCheckBox(
            self.i18n.get("webp", "모든 이미지를 WebP로 일괄 변환")
        )
        self.chk_webp.setCursor(Qt.CursorShape.PointingHandCursor)
        self.chk_webp.setChecked(config.get("webp_conversion", False))

        lbl_webp_desc = QLabel(
            self.i18n.get(
                "webp_desc",
                "모든 이미지를 고효율 WebP 포맷으로 변환하여 확장자 통일성을 보장합니다.",
            )
        )
        lbl_webp_desc.setWordWrap(True)
        lbl_webp_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px; margin-left: 25px;"
        )
        ...
        self.le_viewer_path.setPlaceholderText(
            self.i18n.get("viewer_placeholder", "뷰어 프로그램(.exe) 경로를 선택하세요")
        )
        ...
        viewer_layout.addRow(
            self.i18n.get("viewer_lbl", "뷰어 프로그램:"), viewer_path_layout
        )
        ...
        grp_dup_folders = QGroupBox(
            self.i18n.get("grp_dup_folders_title", "라이브러리 폴더 등록")
        )
        ...
        lbl_dup_desc = QLabel(
            self.i18n.get(
                "dup_folder_desc",
                "관리할 라이브러리 대상 폴더를 추가하세요. (등록된 폴더는 중복 검사에도 사용됩니다.)\nNAS나 대용량 드라이브의 폴더를 지정할 수 있습니다.",
            )
        )
        lbl_dup_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px;"
        )
        grp_dup_layout.addWidget(lbl_dup_desc)

        self.list_dup_folders = QListWidget()
        self.list_dup_folders.setStyleSheet(
            "background-color: #3a3a3a; color: white; border: 1px solid #555; border-radius: 4px;"
        )
        ...
        lbl_idx_desc = QLabel(
            self.i18n.get(
                "setting_update_index_desc",
                "등록된 대상 폴더의 변경사항을 확인하여 인덱스를 동기화합니다.",
            )
        )
        lbl_idx_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px;"
        )
        lbl_idx_desc.setWordWrap(True)

        self.btn_update_index = QPushButton(
            self.i18n.get("setting_update_index", "인덱스 색인 갱신")
        )
        self.btn_update_index.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_update_index.setStyleSheet(
            "background-color: #27AE60; color: white; padding: 6px 12px; border-radius: 4px; font-weight: bold; border: none;"
        )
        ...
        lbl_cache_desc = QLabel(
            self.i18n.get(
                "folder_clear_cache_desc",
                "저장된 모든 중복 파일 매칭 결과 캐시를 초기화합니다.",
            )
        )
        lbl_cache_desc.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px;"
        )
        lbl_cache_desc.setWordWrap(True)

        self.btn_clear_dup_cache = QPushButton(
        ...
        self.btn_clear_dup_cache.setStyleSheet(
            "background-color: #E74C3C; color: white; padding: 6px 12px; border-radius: 4px; font-weight: bold; border: none;"
        )
        ...
        def _make_pw_field(value, placeholder):
            ...
            def toggle_visibility():
                if le.echoMode() == QLineEdit.EchoMode.Password:
                    ...
                    btn.setStyleSheet(
                        "background-color: #3498DB; border: 1px solid #3498DB; border-radius: 4px;"
                    )
                else:
                    le.setEchoMode(QLineEdit.EchoMode.Password)
                    btn.setIcon(qta.icon("fa5s.eye", color="#dddddd"))
        ...
        self.chk_ai_trans = QCheckBox(
            self.i18n.get("ai_trans_enable", "AI를 활용한 공식 영문명 변환 사용")
        )
        ...
        ai_group_layout.addRow(
            self.i18n.get("ai_provider", "AI 모델 선택:"), self.cb_ai_provider
        )

        self.ai_key_widget, self.le_ai_key = _make_pw_field(
            api_keys.get("ai_key", ""), "API Key"
        )
        ai_group_layout.addRow(
            self.i18n.get("ai_api_key", "API Key:"), self.ai_key_widget
        )

        lbl_ai_notice = QLabel(
            self.i18n.get("ai_notice", "해외 DB 검색 시 정확도를 대폭 높입니다.")
        )
        lbl_ai_notice.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px;"
        )
        ...
        self.btn_api_manual = QPushButton(
            self.i18n.get("btn_api_manual", "API 발급 매뉴얼")
        )
        ...
        self.btn_api_manual.setStyleSheet(
            "background-color: #2b5797; color: white; padding: 6px 12px; border-radius: 4px; font-weight: bold; border: none;"
        )
        ...
        self.aladin_widget, self.le_aladin_key = _make_pw_field(
            api_keys.get("aladin", ""), "Aladin TTBKey"
        )
        api_layout.addRow("Aladin TTBKey:", self.aladin_widget)

        self.google_widget, self.le_google_key = _make_pw_field(
            api_keys.get("google", ""), "Google Books API Key"
        )
        api_layout.addRow("Google Books API:", self.google_widget)

        self.vine_widget, self.le_vine_key = _make_pw_field(
            api_keys.get("vine", ""), "Comic Vine API Key"
        )
        ...
        lbl_tag_notice = QLabel(
            self.i18n.get(
                "tag_rules_desc",
                "치환할 태그를 '기존태그 -> 새태그' 형식으로 입력하세요.",
            )
        )
        lbl_tag_notice.setStyleSheet(
            f"color: #aaaaaa; font-size: {self.config['s11']}px;"
        )
        ...
        self.te_tag_rules.setPlaceholderText(
            "Shounen, 소년만화 -> 소년\nAction -> 액션"
        )
        ...
        self.btn_clear_cache = QPushButton(
            self.i18n.get("btn_clear_cache", "검색 캐시 비우기")
        )
        ...
        self.tabs.addTab(
            self.tab_folder_settings,
            self.i18n.get("tab_folder_settings", "폴더 탭 설정"),
        )
        ...
        btn_box.button(QDialogButtonBox.StandardButton.Ok).setText(
            self.i18n.get("btn_save", "저장")
        )
        btn_box.button(QDialogButtonBox.StandardButton.Cancel).setText(
            self.i18n.get("btn_cancel", "취소")
        )
        btn_box.button(QDialogButtonBox.StandardButton.Ok).setCursor(
            Qt.CursorShape.PointingHandCursor
        )
        btn_box.button(QDialogButtonBox.StandardButton.Cancel).setCursor(
            Qt.CursorShape.PointingHandCursor
        )
    ...
    def action_clear_cache(self):
        try:
            with sqlite3.connect(".api_cache.db", timeout=10) as conn:
                ...
                conn.commit()
            Toast.show(
                self, self.i18n.get("msg_cache_cleared", "캐시가 초기화되었습니다.")
            )
        except Exception:
            pass
    ...
    def action_clear_dup_cache(self):
        reply = QMessageBox.question(
            ...
            self.i18n.get("folder_clear_cache", "중복 매칭 캐시 초기화"),
            self.i18n.get(
                "folder_clear_cache_confirm",
                "저장된 모든 중복 매칭 결과 캐시를 삭제하시겠습니까?\n(매칭 속도가 일시적으로 느려질 수 있습니다.)",
            ),
    ...
    def browse_viewer_path(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Viewer Program", "", "Executable Files (*.exe);;All Files (*)"
        )
        if path:
    ...
    def add_dup_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, self.i18n.get("dlg_sel_dup_folder", "라이브러리 폴더 선택")
        )
        if folder:
            # 중복 방지
            items = [
                self.list_dup_folders.item(i).text()
                for i in range(self.list_dup_folders.count())
            ]
    ...
    def action_update_index(self):
        ...
        if hasattr(main_win, "tab_folder"):
            ...
            Toast.show(
                self,
                self.i18n.get("setting_update_index_msg", "인덱스가 갱신되었습니다."),
            )
        ...
        if ext in [".zip", ".cbz"]:
            with zipfile.ZipFile(filepath, "r") as zf:
                ...
                if img_files:
                    img_files.sort()
                    cover = next(
                        (f for f in img_files if "cover" in f.lower()), img_files[0]
                    )
...
class ConflictResolutionDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        lbl_desc = QLabel(
            self.i18n.get(
                "lbl_conflict_desc", "이동할 경로에 이미 같은 이름의 파일이 존재합니다."
            )
        )
        ...
        compare_layout.addWidget(
            create_card(
                self.i18n.get("col_source", "Source (이동할 파일)"),
                src_info,
                self.src_path,
            )
        )
        compare_layout.addWidget(
            create_card(
                self.i18n.get("col_dest", "Destination (기존 파일)"),
                dest_info,
                self.dest_path,
            )
        )
        ...
        btn_over.setStyleSheet(
            "background-color: #E74C3C; color: white; border-radius: 4px; padding: 6px 15px;"
        )
        btn_over.clicked.connect(lambda: self.done(1))

        btn_ren = QPushButton(self.i18n.get("btn_rename_new", "새 이름으로 저장"))
        btn_ren.setStyleSheet(
            "background-color: #3498DB; color: white; border-radius: 4px; padding: 6px 15px;"
        )
        btn_ren.clicked.connect(lambda: self.done(2))

        btn_skip = QPushButton(self.i18n.get("btn_skip", "건너뛰기"))
        btn_skip.setStyleSheet(
            "background-color: #555; color: white; border-radius: 4px; padding: 6px 15px;"
        )
...
class MoveToLibraryDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        opt_layout.addWidget(
            QLabel(self.i18n.get("lbl_select_lib", "이동할 라이브러리 선택:"))
        )

        self.cb_lib = QComboBox()
        self.cb_lib.addItems(self.libraries)
        self.cb_lib.setStyleSheet(
            "QComboBox { background-color: #1e1e1e; border: 1px solid #555; padding: 4px; }"
        )
        ...
        self.chk_folder = QCheckBox(
            self.i18n.get("chk_create_folder", "현 폴더 이름으로 폴더 생성하고 이동")
        )
        ...
        layout.addWidget(
            QLabel(self.i18n.get("lbl_preview_move", "이동 경로 미리보기:"))
        )
        ...
        self.table.setHorizontalHeaderLabels(
            [
                self.i18n.get("col_source_path", "Source (현재 경로)"),
                self.i18n.get("col_dest_path", "Destination (이동할 경로)"),
            ]
        )
        ...
        self.table.setStyleSheet(
            "QTableWidget { background-color: #1e1e1e; gridline-color: #444; } QHeaderView::section { background-color: #333; }"
        )
        ...
        btn_ok.setStyleSheet(
            "background-color: #3498DB; color: white; border-radius: 4px; padding: 6px 20px;"
        )
        btn_ok.clicked.connect(self.accept)
        btn_cancel = QPushButton(self.i18n.get("btn_cancel", "취소"))
        btn_cancel.setStyleSheet(
            "background-color: #555; color: white; border-radius: 4px; padding: 6px 20px;"
        )
    ...
    def update_preview(self):
        ...
        for i, plan in enumerate(self.move_plans):
            ...
            if self.is_folder_mode:
                ...
                if create_folder:
                    dest = os.path.join(
                        target_lib, plan["current_dir_name"], plan["base_name"]
                    )
                else:
                    dest = os.path.join(target_lib, plan["base_name"])

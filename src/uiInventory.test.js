...
test('14.3 input 전수 목록이 실제 제어와 연결되어 있다', () => {
    ...
    assertInventory('metadata', [
        ...
        ['메타데이터 저장 시리즈 그룹 후보 상태', 'savedSeriesGroupOptions'],
        ['메타데이터 DB 시리즈 그룹 후보 반영', 'rememberSeriesGroupOptions(result.seriesGroupOptions || [])'],
        ['메타데이터 저장 성공 시리즈 그룹 후보 반영', 'savedGroupTargets.map(item => item.metadata?.SeriesGroup)'],
...
test('리스트 파일 강제 업데이트는 선택 파일을 단일 파일 미리보기로 갱신한다', () => {
    assertInventory('folder', [
        ...
        ['보이는 항목 표지 캐시 우선 로드', "getFilePreview(coverPreviewFilePath(file), { force: false })"],
        ['보이는 항목 표지 결과 캐시 반영', 'context.updateCachedFiles(context.folderPath, context.scanOptions'],

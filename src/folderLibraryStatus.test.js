...
const messages = {
    ...
    folder_library_scan_meta: '{0}개 · {1}',
...
test('최신 인덱스 상태의 라이브러리는 새로고침 아이콘을 표시하지 않는다', () => {
    ...
    assert.equal(libraryStatusText(t, state, { nowMs }), '30개 · 1분 전');
...
test('업데이트가 필요한 라이브러리만 새로고침 아이콘을 표시한다', () => {
    ...
    assert.equal(libraryStatusText(t, state), '업데이트 필요');
...
test('heartbeat가 있는 scanning 상태는 인덱싱 중으로 표시한다', () => {
    ...
    assert.equal(libraryStatusText(t, state, { nowMs }), '인덱싱 중');
    assert.equal(shouldShowLibrarySyncButton(state, { nowMs }), false);
});
...
test('중단된 라이브러리 스캔은 스캔 중으로 표시하지 않는다', () => {
    ...
    assert.equal(libraryStatusText(t, state), '스캔 중단됨');
    assert.equal(shouldShowLibrarySyncButton(state), true);
});
...
test('heartbeat가 오래된 scanning 상태는 중단 상태로 처리한다', () => {
    ...
    assert.equal(libraryStatusText(t, state, { nowMs, heartbeatTimeoutMs: 120000 }), '스캔 중단됨');
});
